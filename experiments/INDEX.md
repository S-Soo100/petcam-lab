...
| 2026-07-19 | **care-label-audit** daily 케어행동 라벨 전수 육안감사 (Sonnet v4.0, 11건 전수) | `reject 케어 VLM 신뢰 / confirm 비-VLM 필요` | daily 워커 non-moving/unseen 라벨 전수 11건 = **10/11 오답(91%)**. 정답은 conf 0.85 hand_feeding 1건뿐(conf순≠정오). Failure A=dish-presence confabulation 9건(밥그릇 근처 이동→먹는 근거 지어냄), B=카메라 주야간 모드전환 가짜 트리거 1건(캡처+분석 2겹 오탐). **격리 검증: daily 오탐 0건 앱 노출**(clip_vlm_jobs 격리 계약 유효). 열린 리스크=behavior_logs vlm eating/drinking 46건 terra-server 경로 미확인 | [vlm-care-label-audit-20260719/REPORT.md](vlm-care-label-audit-20260719/REPORT.md) |
| 2026-07-21 | **T1 highlight-selection probe** (DB-only 합성점수 top20 vs random20, 사람 blind, LLM 0회) | `reject` (점수식 v1 무효) | S informative 50% vs R 45% = **Δ+5%p** < 게이트 +10%p. **S absent 30%**(안전점검 발동) — detector 오검출이 존재(observed_sec)+주기성(autocorr) 동시 오염, S-absent 5/6이 Cam 2 동일 시그니처(전체관찰·autocorr 0.8) = 카메라 고유 상시 오염원. T0 "존재 유효"는 Cam 2 제외 덕(카메라 의존). Cam 3은 잘 작동(absent 0/12·inf 8/12). v2 후보=오검출 시그니처 페널티·카메라 정규화·Gate prelabel 결합(새 시험지 필요). GT 40건 적립(누적 120) | [t1-highlight-selection/REPORT.md](t1-highlight-selection/REPORT.md) |
| 2026-07-21 | **T0 bowl-dwell probe** (체류=케어 신호 선검증, blind 80건, 비-VLM, owner 육안) | `reject` (체류-단독 무효) | top60 케어 **0**/59 vs random20 **0**/20 — H1 기각. 체류 상위군은 그릇 근접(dwell median 34.3s vs 0.0s, absent 3% vs 55%)은 정확히 잡으나 그 중 **near_bowl_no_care 20건(34%)**·drinking 0건. VLM confabulation과 **동일 실패모드를 비-VLM에서도 재현**. reject=체류-**단독** 기각(결합 feature 여지·hard neg 21건 확보). eating은 밥그릇 부재로 미검증. 가장 비싼 GT 투자를 가설 뒤로 둔 순서 결함 해소 | [t0-bowl-dwell-probe/REPORT.md](t0-bowl-dwell-probe/REPORT.md) |
