...
| 2026-07-14 · 활동필터 preflight+audit+safety(0714) | [activity-preflight-0714/REPORT.md](activity-preflight-0714/REPORT.md) | 0.25=reject→**audit로 threshold 문제 규명**(conf0.14~0.21 걸러진것, Gate v3는 일반화/segment엔 여전히 필요). v1(0.10) 재실험 후 **fresh safety holdout(겹침0·24)**: **exclude_absent REJECT**(FE2 카메라B, 0.10도 게코놓침), exclude_static HOLD(active오제외0이나 11/12가 한 에피소드=effective≈2). **사람 질문이 presence/product-outcome 혼합**(static 12는 게코 보임). utility clip가중 27%(static17+absent10, detector upper bound). 두 스위치 disabled → **C2 shadow launchd 상시가동**(settings 3·activity-v1, 첫 RunAtLoad assessments 205↑, **effective==raw·실제 차감 0**). §8~15 |
