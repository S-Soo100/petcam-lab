...
**2026-06-17 업데이트 — 자매 레포 2개 추가 편입:** **gecko-vision-gate**(`~/myPythonProjects/gecko-vision-gate`, Gate 상시 prelabeler) · **petcam-nightly-reporter**(`~/petcam-nightly-reporter`, Reporting 공장). **2026-06-20 — petcam-mac-runner 편입(4번째):** **petcam-mac-runner**(`~/petcam-mac-runner`, GitHub `S-Soo100/petcam-mac-runner`, 맥미니 폴링 워커 공통 골격 = 참조 스켈레톤. cron 불가→launchd 필수, 메모리 `cron-launchd-keychain`). 통합 아키텍처 SOT = `tera-ai-product-master/docs/specs/petcam-ai-pipeline.md §11`. ⚠️ rba-worker 외 이 **세 레포**도 petcam-lab 에서 직접 안 건드린다 — 각 레포에서 작업.
...
| 맥미니 폴링 워커 공통 골격 (스케줄링 launchd + Claude headless + Slack) | **petcam-mac-runner** (`~/`, GitHub `S-Soo100/petcam-mac-runner`) |
...
0. **🚦 새 방향/투자/실험 제안인가?** → [`docs/decision-gate.md`](docs/decision-gate.md) 4게이트(①SOT 부합 — 실제로 열어 확인 ②기대효과 명확 ③측정가능 ④유효한 계획) 먼저 통과시키고 판정을 로그에 append. 탈락 제안 재등판이면 탈락 사유 해소부터. 이 로그는 Codex 등 타 에이전트와 공유되는 의사결정 기록 (기존 스펙 진행·단발 버그픽스는 해당 없음).
...
| Codex/local router research | `petcam-lab` | Codex, local text LLM, OpenCV feature JSON | **중단/exploratory**. metadata·provenance·review 인프라 유지, future holdout 비용 계약은 `router-cost-v2` | 기존 결과를 adoption 근거로 재사용하거나 Claude 연구 결과를 덮어쓰지 않기 |
...
- **2026-07-12 최신 판정:** local router v0/v1/v2와 care-guard v1/v1.1은 `invalid-for-adoption`, 비용 절감 `not-measured`, production eligibility `rejected`. 추가 threshold 튜닝 금지. 다음 평가는 baseline API 모델·입력·prompt·비용과 router policy를 먼저 동결하고 승인 이후 future camera-night만 사용한다.
