...
If a human hand/finger, spoon, syringe, tweezers/forceps, pipette, or any artificial feeding tool is VISIBLE in the frame at the moment of the activity → classify as `hand_feeding`, no matter what the lizard appears to be doing (even if it looks like eating_prey, eating_paste, or drinking). `hand_feeding` marks human-assisted feeding, which is out-of-distribution for autonomous monitoring. Only when NO human or tool is visible in the frame do the decision rules below apply. (If a tool appears for only part of the clip and then leaves, judge by what is visible during the dominant portion of the activity.)
