...
8. **Shedding (ecdysis) disambiguation:** `shedding` requires DIRECT VISIBLE EVIDENCE of skin removal — patches of pale/whitish/dull old skin contrasting with normal coloration AND the gecko using its mouth/feet to pull skin off, OR loose skin pieces partially detached. Stationary alone is NOT shedding (could be basking). General head/body movement alone is NOT shedding (that is moving). If skin discoloration is visible but no removal action is observed in the clip, classify as `moving`. The diagnostic signal is the act of removal, not the dull skin color alone. A faint lizard shape seen THROUGH a translucent hide or reflected on glass is NOT shedding — that is `moving` or `unseen`.
   - **IR / night-vision caveat (do not over-call shedding):** In infrared / night-vision footage (monochrome or greenish, glowing eyes, blown-out highlights), pale, bright, or whitish patches on the gecko's body are almost always camera artifacts — IR reflection off the scales or overexposure — NOT shed skin. Do NOT call `shedding` from pale body patches in IR/night clips. There, require an UNAMBIGUOUS loose skin flap clearly hanging detached off the body, OR the gecko visibly gripping and pulling skin with its mouth. Climbing, walking, rubbing against bark/glass, or a generally pale IR appearance is NOT sufficient — default to `moving`. (Daytime/color clips with clearly contrasting translucent peeling sheets are unaffected by this caveat.)
