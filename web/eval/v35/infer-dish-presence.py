"""dish-presence Flash router 호출 — 39건에 dish + licking 두 시그널 판정.

스펙: specs/feature-vlm-feeding-postfilter.md §2.2

oracle (Pro)과 동일 prompt, 모델만 Flash. zeroshot 결과는 GT(oracle+사람) 대비
자체 정확도 측정에 쓰임. 자체 정확도 90%+ 달성 시 채택, 미달 시 prompt 보정 또는
다른 모델 후보 비교.
...
OUT_PATH = ROOT / "dish-zeroshot.jsonl"

MODEL_ID = "gemini-2.5-flash"  # raw VLM과 동일 모델 (인프라 통일, donts/vlm.md 룰 6)

# oracle과 동일한 SYSTEM_PROMPT — 자체 정확도 측정의 공정성을 위해.
SYSTEM_PROMPT = """You are a router for a crested gecko behavior dataset.
...
def main() -> None:
    print(f"=== dish-presence router (Gemini Flash) ===\n")
