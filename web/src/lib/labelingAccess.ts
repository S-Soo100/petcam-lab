...
// 본 큐·일반 clip 접근용 가드(설계 §12). 예전엔 requireLabelingAccess(멤버십) 위에 active
// 튜토리얼 완료/면제 게이트(403 tutorial_required)를 얹었으나, 대화형 튜토리얼 트랙이
// 2026-09-07 owner 결정으로 퇴역해(docs/decision-gate.md 4차) 지금은 멤버십 판정과 같다.
// 호출부(~10곳)를 건드리지 않으려고 이름은 유지한다.
export async function requireProductionLabelingAccess(
  req: NextRequest,
): Promise<LabelingAccessResult> {
  return requireLabelingAccess(req);
}
