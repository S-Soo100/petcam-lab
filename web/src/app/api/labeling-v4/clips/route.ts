...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await supabaseAdmin.rpc('fn_list_labeling_v4_clips', {
      ...
      p_label_state: parsed.labelState, p_highlight_state: parsed.highlightState, p_behavior_flag: parsed.behaviorFlag, ...(parsed.sampleId ? { p_sample_id: parsed.sampleId } : {}),
