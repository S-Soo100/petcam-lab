...
const EMPTY = { flagged: false, flagged_by_name: null, flagged_at: null };
const MARKS = { meaningful: EMPTY, wheel: EMPTY, fall: EMPTY, closeup: EMPTY };
vi.mock('../../_behavior-flag', () => ({ loadBehaviorMarks: async () => MARKS }));
...
describe('GET /api/labeling-v4/clips/[clipId]', () => {
  it('clip 메타 + highlight, r2_key 비노출', async () => {
    ...
    expect(body).toEqual({ id: CLIP, camera_id: 'c1', started_at: '2026-09-08T00:00:00Z', duration_sec: 60, media_ready: true, highlight: expect.objectContaining({ current: expect.any(Object), initial: expect.any(Object) }), behavior_flag: EMPTY, behavior_marks: MARKS, featured: null });
