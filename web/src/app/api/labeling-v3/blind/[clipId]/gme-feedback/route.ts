...
function mapRpcError(error: { code?: string } | null): NextResponse {
  ...
  return response('GME 오류 기록 저장에 실패했어. 잠시 후 다시 시도해.', 'save_failed', 502);
}
...
export async function POST(req: NextRequest, { params }: { params: { clipId: string } }) {
  ...
  const feedbackKind = input.feedback_kind;
  ...
  if (
    ...
    return response('오류 종류, 영상 시각 또는 GME 버전이 잘못됐어.', 'invalid_request', 400);
  }
  ...
  const { data, error } = await supabaseAdmin.rpc('fn_append_motion_clip_gme_feedback', {
    p_event_id: randomUUID(),
    p_clip_id: params.clipId,
    p_reviewer_id: access.userId,
    p_feedback_kind: feedbackKind,
    p_surface: access.scope.cohortKind === 'canary' ? 'blind_canary' : 'blind_live',
  ...
  return NextResponse.json({ status: 'recorded', timestamp_sec: Number(row.timestamp_sec) });
}
