...
// Owner 짧은 영상 자동 제외 검수·복구 화면(설계 §4·§6.3).
//
// 검증된 카메라의 장치 오류 후보(quarantined)를 카드로 보여주고, 정상 영상이면 라벨 대상으로
// 복구한다. 복구는 다른 탭으로 이동하지 않고 카드만 활성 목록에서 제거한다(설계 §4 5). 7일 뒤
// 안전 삭제가 끝난 카드(media_deleted)는 `원본 삭제됨 · 메타데이터 보존` + 재생 비활성으로 표시한다.
...
const RESTORE_REASON = '정상 영상으로 확인되어 라벨 대상으로 복구';
    ...
    <section className="flex flex-col gap-4">
      <div className="flex flex-col gap-1">
        ...
        <p className="break-words text-sm text-zinc-600">
          검증된 카메라에서 장치 오류로 걸러진 영상이야. 정상 영상이면 다시 라벨 대상으로 되돌릴 수 있고,
          7일 뒤 안전 삭제가 끝나면 원본 없이 기록만 남아.
        </p>
      </div>
      {items.length === 0 ? (
        <Card padding="lg">
...
export default function AutoExcludedList() {
  ...
  const onRestore = useCallback(async (clipId: string) => {
    ...
    try {
      ...
      else setErr('복구에 실패했어. 잠시 후 다시 시도해.');
    } finally {
