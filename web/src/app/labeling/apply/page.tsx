...
// 라벨러 참여 신청 — 로그인은 했지만 아직 신청 row 가 없는 사용자용(설계 §3.3 미승인 단일 화면).
//
// - Auth 이메일은 읽기 전용으로 보여준다(서버가 신뢰하는 값).
// - 이름만 입력받아 POST /api/labeler-applications. 이 신청이 화면의 유일한 다음 행동이다.
// - 성공하면 접근 상태를 갱신하고 /labeling/pending 으로 이동.
// - 업무 내비게이션은 없다(RoleShell 이 미승인 역할엔 nav 를 렌더하지 않음). 비밀번호 변경·
//   로그아웃은 셸 상단 계정 메뉴로만 접근한다.
//
