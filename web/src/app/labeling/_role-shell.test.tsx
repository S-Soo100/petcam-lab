...
describe('RoleShell 메뉴 계약(설계 §3)', () => {
  ...
  it('이어짐 확인은 boundary 배정이 있을 때만 보인다', () => {
    expect(render('labeler')).not.toContain('이어짐 확인');
    expect(render('labeler', '/labeling/boundary', true)).toContain('이어짐 확인');
  });
...
describe('RoleShell 반응형 class 계약(설계 §9)', () => {
  it('하단 메뉴는 항목 수 동적 grid + lg 사이드 그리드 + 잘림 방지 토큰', () => {
    const html = render('labeler', '/labeling', true);
    ...
    expect(html).toContain('max-w-[1484px]');
