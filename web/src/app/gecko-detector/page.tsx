...
import { labelingAssistEnabled } from '@/lib/yoloDemoRoute';
import { DetectorDemo } from './_detector-demo';

export default function DetectorPage() {
  const assistEnabled = labelingAssistEnabled(process.env);
  return (
      ...
      <div className="mx-auto max-w-5xl space-y-8">
        <header className="space-y-3">
          ...
          <p className="max-w-3xl text-zinc-600">
            {assistEnabled
              ? '사진이나 짧은 영상을 올리면 development-only 라벨링 보조 worker의 후보 bbox를 보여줘. 게코 없음 판정이 아니야. 실제 모델 버전과 threshold는 처리 결과에서 확인해.'
              : '사진이나 짧은 영상을 올리면 versioned bbox 결과를 보여줘. 지금 연결된 fake는 화면과 worker 계약을 검증하는 시연용이며 실제 YOLO checkpoint가 아니야.'}
          </p>
        </header>
        <DetectorDemo assistEnabled={assistEnabled} />
      </div>
    </main>
  );
}
