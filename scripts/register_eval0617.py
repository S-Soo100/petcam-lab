"""eval-0617 신규 평가셋 등록 — R2 업로드 + camera_clips + behavior_logs + manifest 행 추가.

Downloads/new-data-2026-06-17 의 10개 영상(drinking7/eating-paste2/hand-feeding1)을
clips/eval-0617/ 로 올리고 production DB + 평가 manifest 에 등록한다.

eval-0608/0615 선례 계승 + 차이:
- R2/manifest 네이밍 = eval-0615 패턴 `{gt}__na__{clip8}.mp4` (clip8 = clip_id 앞 8자).
  manifest filename 과 r2_key 가 일치 → grep/조회 일관. (메모리 r2-clip-id-uuid-mismatch 교훈)
- manifest.csv(storage/dataset-203/) 행까지 추가 — 평가 SOT 가 manifest
  (`_extract_frames_clip --from-manifest` 가 읽음), DB 는 production SOT. 둘 다 채운다.
- 회귀셋 185 동결 유지: source=eval-0617 로 187→197 전체에만 포함.
  버전 paired 회귀(185)엔 안 들어감 (노이즈/신규 샘플 회귀셋 혼입 방지, CLAUDE.md 룰4).
- quality_tag 빈칸 — 사용자 육안 직접 태깅(self-bias 방지). 전부 1206x2622 세로 handheld.
- 멱등: camera_clips.file_path 에 원본 파일명 있으면 skip (재실행 안전).

실행:
  PYTHONPATH=. uv run python scripts/register_eval0617.py            # dry-run (출력만)
  PYTHONPATH=. uv run python scripts/register_eval0617.py --apply    # 실제 업로드+INSERT+manifest
"""
...
INBOX = Path("/Users/baek/Downloads/new-data-2026-06-17")
R2_PREFIX = "clips/eval-0617"
MANIFEST = REPO / "storage" / "dataset-203" / "manifest.csv"
SOURCE = "eval-0617"
OWNER_USER_ID = "380d97fd-cb83-4490-ac26-cf691b32614f"  # owner (probe 확인, eval-0608 동일)
...
def existing_src_names() -> set[str]:
...
def main() -> int:
    ...
    print(f"{INBOX.name} 영상 {len(videos)}개, mode={'APPLY' if APPLY else 'DRY-RUN'}")

    already = existing_src_names()
    ...
    for p in videos:
        ...
        log_row = {
            ...
            "source": "human",
            "verified": True,
            "notes": f"eval-0617 GT (파일명 힌트, src={p.name})",
        ...
        # 1. R2 업로드 (멀티파트 자동 — 4K 100MB+ 대비, put_object 단일 부적합)
        client.upload_file(
        ...
        # 4. 로컬 평가용 복사 — _extract_frames_clip 이 storage/dataset-203/{filename} 를 로컬에서 읽음
    ...
    if not APPLY:
        print("\n검수 후 실제 실행: PYTHONPATH=. uv run python scripts/register_eval0617.py --apply")
    else:
