"""Formal Blind30 원자 예약 RPC의 local-only PostgreSQL 실증 runner."""
...
BLOCKED_VERDICT = "FORMAL30_INFRA_BLOCKED_DB_RUNTIME"
REQUIRED_MARKER = "FORMAL30_PROBE_OK"
_APPLY_ORDER = (
    ...
    ("formal30", "migrations/2026-07-31_motion_blind_formal30.sql"),
)
_PROBE_SQL = "tests/sql/motion_blind_formal30_probe.sql"
_RESIDUE_TABLES = (
    "public.motion_clips",
    "public.cameras",
...
def formal30_temp_database_name(token: str) -> str:
    return f"blind_probe_formal30_{token}"


def validate_formal30_temp_database_name(name: str) -> None:
    if not re.fullmatch(r"blind_probe_formal30_[a-f0-9]+", name):
...
def missing_marker(stdout: str) -> str | None:
    return None if REQUIRED_MARKER in stdout else REQUIRED_MARKER
...
def _run_steps(
    ...
    result = backend.psql_run(probe.read_text(), timeout=120.0)
    if result.returncode != 0:
        detail = (result.stderr.strip() or result.stdout.strip())[:1200]
        raise ProbeFailed(f"probe_failed: {detail}")
    absent = missing_marker(result.stdout)
    if absent is not None:
        raise ProbeFailed(f"marker_absent:{absent}")
...
def run_local_formal30_probe(
    ...
    *,
    ...
    name = formal30_temp_database_name(secrets.token_hex(8))
    validate_formal30_temp_database_name(name)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Formal Blind30 local DB probe")
    ...
    try:
        return run_local_formal30_probe(
            paths,
            probe,
            pg_bin=args.pg_bin,
            host=args.pg_host,
        ...
        print(f"FORMAL30_PROBE_FAILED: {exc}", file=sys.stderr)
        return 1
