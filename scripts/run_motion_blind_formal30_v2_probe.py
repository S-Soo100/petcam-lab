"""Formal Blind30 v2 원자 예약 RPC의 local-only PostgreSQL 실증 runner."""
...
BLOCKED_VERDICT = "FORMAL30_V2_INFRA_BLOCKED_DB_RUNTIME"
REQUIRED_MARKER = "FORMAL30_V2_PROBE_OK"
_APPLY_ORDER = (
...
_PROBE_SQL = "tests/sql/motion_blind_formal30_v2_probe.sql"
_RESIDUE_TABLES = (
    "public.motion_clips",
    "public.cameras",
...
def formal30_v2_temp_database_name(token: str) -> str:
    return f"blind_probe_formal30_v2_{token}"


def validate_formal30_v2_temp_database_name(name: str) -> None:
    if not re.fullmatch(r"blind_probe_formal30_v2_[a-f0-9]+", name):
...
def run_local_formal30_v2_probe(
    paths: list[tuple[str, Path]],
    probe: Path,
    *,
    ...
    name = formal30_v2_temp_database_name(secrets.token_hex(8))
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Formal Blind30 v2 local DB probe")
    ...
    try:
        return run_local_formal30_v2_probe(
            paths,
            probe,
            pg_bin=args.pg_bin,
            host=args.pg_host,
        ...
        print(f"FORMAL30_V2_PROBE_FAILED: {exc}", file=sys.stderr)
        return 1
