...
분석은 202 통합건(159 + 44 eval-0608 − hiding/편집영상 정리)을 두 그룹으로 분리:
- **P0/기타** (GT 가 hand_feeding 아님): feeding-merged 정확도 → v3.5 floor 85.5% 와
  근사 비교 (OOD 제외 P0 부분 — 분모 다르므로 직접 비교 아닌 추세 판단용).
- **OOD** (GT=hand_feeding): v3.6 가 hand_feeding 으로 잡은 비율 (recall). v3.5 는
  클래스 자체가 없어 구조적으로 0 → v3.6 의 순수 이득.

재실행 안전 (JSONL 누적, 성공 clip skip). 결과 파일은 버전별 분리:
v3.6 → /tmp/vlm-regression-v36.jsonl, v3.6.1 → /tmp/vlm-regression-v361.jsonl

실행: PYTHONPATH=. uv run python scripts/eval_vlm_v36_handfeeding.py [--version v3.6|v3.6.1]
"""
...
def out_path_for(prompt_version: str) -> Path:
    """버전별 결과 파일 분리 — v3.6 / v3.6.1 결과가 안 섞이게 (점 제거: v36 / v361)."""
    return Path(f"/tmp/vlm-regression-{prompt_version.replace('.', '')}.jsonl")
...
def run_inference(targets, prompt_version: str, out_path: Path) -> None:
...
def analyze(prompt_version: str, out_path: Path) -> None:
    if not out_path.exists():
    ...
    print("\n" + "=" * 64)
    print(f"v3.6 계열 평가 — N={n} (실패 {failed})  [prompt={prompt_version}]")
    print("=" * 64)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="v3.6 계열(hand_feeding OOD) 회귀평가 — feature-hand-feeding-ood-label.md C-3"
    )
    parser.add_argument(
        "--version",
        choices=["v3.6", "v3.6.1"],
    ...
    out_path = out_path_for(prompt_version)
    logger.info("프롬프트 버전 %s → 결과 파일 %s", prompt_version, out_path)

    targets = load_eval_set()
    if len(targets) != 202:
        logger.warning(
            "평가셋 %d (202 통합 가정 — 159 + 44 eval-0608 − hiding/편집 정리)", len(targets)
        )
    run_inference(targets, prompt_version, out_path)
    analyze(prompt_version, out_path)
    return 0
