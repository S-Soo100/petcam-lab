...
라벨러 피드백상 현재 검은색 active chip은 버튼인지, 선택된 상태인지, 비활성 상태인지 바로 구분되지 않는다. 라벨링 화면의 필터·선택 카드·판정 CTA는 다음 공통 상태를 사용한다.

| 상태 | 화면 표현 |
|---|---|
| 누를 수 있음 | 흰 배경, 선명한 중립 테두리, 약한 그림자, pointer/hover |
| 선택됨 | 연한 의미색 배경, 2px 의미색 테두리, 체크 아이콘과 `선택됨` 텍스트 |
| 주요 실행 | 검은색 대신 진한 초록 배경, 흰 글자, 동사형 문구 |
| 비활성 | 회색 배경·테두리와 낮은 대비, `선택됨` 표시 없음 |
| 위험 작업 | 연한 빨강 또는 빨강 outline, 확인 문구 유지 |

- `label` 선택은 초록, `hold`는 주황, `exclude`는 빨강 계열을 사용한다.
- 필터·날짜·카메라 chip의 선택 상태는 연한 초록+체크로 통일한다.
- 색상만으로 상태를 전달하지 않고 체크·텍스트·`aria-pressed`를 함께 쓴다.
- 모든 주요 컨트롤은 최소 높이 44px, 카드 전체 클릭, 키보드 focus ring을 제공한다.
- 검은색 active 채움은 라벨링 필터·선택·판정 컨트롤에서 사용하지 않는다.
- 로그인·회원관리·일반 내비게이션의 시각 개편은 이번 범위 밖이다.

체험 흐름:

`[화면]` 흰 배경과 테두리가 있는 컨트롤이 버튼처럼 보인다.

`[조작]` 라벨러가 선택한다.

`[반응]` 의미색 배경·굵은 테두리·체크와 `선택됨`이 동시에 나타난다.

`[감정]` 색을 구분하지 못해도 무엇을 눌렀고 현재 무엇이 선택됐는지 확신한다.

### 4.7 쳇바퀴 상호작용 입력

`올라타기·밀기`는 접촉 방식이고 `회전시키기`는 결과이므로 여섯 항목을 같은 위계에 나열하지 않는다. 저장 enum과 payload는 유지하되 관찰 순서대로 묻는다.

1. `게코가 쳇바퀴 위나 안에 올라가 있었어?` → `ride`
2. `쳇바퀴가 실제로 돌아갔어?` → `rotate`
3. `게코가 밖에서 쳇바퀴를 밀거나 건드렸어?` → `push`
4. `다른 행동도 기록하기`를 펼치면 `chase / repeated_return / other`를 보여준다.

각 질문은 큰 예/아니오형 선택 카드로 표시하고 복수 선택을 허용한다. 선택 결과는 `기록된 내용: 쳇바퀴 위·안에 올라갔고, 쳇바퀴가 실제로 돌아감`처럼 자연어로 확인시킨다.

`떠남`은 새 interaction enum이 아니라 상호작용 구간의 끝이다. 쳇바퀴 구간 종료시간 바로 아래에 다음 문구를 항상 표시한다.

> 게코가 쳇바퀴에서 내려온 뒤 더 이상 쳇바퀴와 상호작용하지 않는 순간을 종료 시점으로 표시해.
>
> 내려온 뒤에도 밖에서 밀거나 건드리면 상호작용이 계속되는 중이야.

그냥 떠나면 구간만 종료한다. 떠난 뒤 일반 이동이 보이면 `moving` 구간을 별도로 기록하고, 떠났다가 다시 돌아온 경우에만 `repeated_return`을 선택한다.
...
- 선택 상태는 검은 배경이나 색상 하나에 의존하지 않고 체크·테두리·텍스트로 중복 표현한다.
- 쳇바퀴 질문은 접촉 방식·회전 결과·보조 행동을 다른 위계로 보여준다.
- 쳇바퀴 구간 종료 입력 바로 아래에 떠남/계속 상호작용 기준이 보인다.
...
- 그룹 진행 현황
- 초보자 안내·빈 상태·복구 UX
- 라벨링 필터·선택·판정 버튼의 공통 상태 디자인
- 쳇바퀴 상호작용 단계형 질문과 종료시점 안내
- API/DB 권한과 감사 이력
