...
일반 라벨링 큐의 정본 `camera_clips`와 Universal Python Evidence의 정본 `motion_clips`는 서로 다른
clip 집합이다. 2026-07-15 live 감사에서도 ID 교집합은 0건이었다. 따라서 Evidence GT는
`motion_clips`를 정본으로 사용하며, 기존 `/api/clips/[id]/file/url`이나 `camera_clips` FK를 재사용하지
않는다. 후보 clip을 `camera_clips`로 복사·미러링하는 것도 이 연구의 부수효과로 금지한다.

후보는 `duration_sec > 0`이고 `r2_key`가 비어 있지 않은 `motion_clips`만 허용한다. `r2_key`는
SELECT-only probe 내부에서 재생 가능성 판정에만 사용하고 aggregate/per-clip artifact·hash·API 응답에는
기록하지 않는다. B2의 owner-only media route가 candidate의 `motion_clips` row를 서버에서 다시 조회해
현재 Web R2 client로 짧은 signed URL을 발급한다. preview에서 실제 영상 재생까지 확인하지 못하면
production으로 진행하지 않는다.

SELECT-only probe가 읽을 수 있는 정본:

- `motion_clips` (`id`, `camera_id`, `started_at`, `duration_sec`, `r2_key`)
...
`priority_score`, `reason_codes`, `source_run_id`, `source_assessment_id`를 가진다. `priority_score`는
strata 내부 정렬을 완료한 뒤 `1 - rank / max(count - 1, 1)`로 정규화한 결정론적 실수다. 원래 정렬
component와 tie-break는 selector version에 고정하고 manifest identity에 포함한다. 이 값은 검수
우선순위일 뿐 GT가 아니다. 행동 GT가 필요한 semantic strata 4·5에서는 기존 사람 라벨을 retrieval
신호로만 쓸 수 있으며 evidence 5축 값으로 복사하지 않는다.
...
- signed media URL route는 candidate가 가리키는 `motion_clips`를 service-role로 다시 읽고 product owner를
  검증한 뒤 `r2_key`를 응답에 넣지 않은 채 URL만 반환한다. `camera_clips` 권한 helper는 사용하지 않는다.
