...
120번째 자격 판정과 phase 전환은 한 DB transaction에서 실행한다.

1. `eligible` 개수를 센다.
2. 60개 이상이면 해당 pair 전체에 owner/peer assignment를 생성하고 `development_open`으로 전환한다.
...
- 동일 clip은 왼쪽/오른쪽 이웃 경계에 겹쳐 등장할 수 있고 이것이 의도된 핵심 조건
