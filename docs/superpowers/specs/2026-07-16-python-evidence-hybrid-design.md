...
> **상태:** 설계 정본 / S1R2 통과 / Universal Python Evidence Worker 구현계획 승인
> **작성:** 2026-07-16
...
> **S2 정본:** [`2026-07-17-python-evidence-universal-worker-design.md`](2026-07-17-python-evidence-universal-worker-design.md). 모든 `motion_clips`를 durable queue에 등록하되 adaptive Level 0/1 raw evidence만 만들고 의미 행동·selector 적용은 금지한다. 과거 activity-worker 확장안은 SUPERSEDED다.
...
2. **gap 필드를 gate 경로 확장 vs router_features 확장 어디에 둘지** — dwell/ROI series가 detector 의존이면 gate, OpenCV만이면 router_features. S1 벤치마크가 결정.
   - **[2026-07-17] S1 = `S1_HOLD_RUNTIME_BUDGET` → recovery = `S1R_HOLD_INCOMPLETE`(S2 blocked, 이력 additively 보존).** recovery 는 A6 확정 원인(실행 PATH `/opt/homebrew/bin` 누락)을 fail-closed preflight 로 고치고 동결 workload 를 완주 시도했다. MPS window 1(333 records)에서 A6 는 29/32 clip 정상이었으나 **3 clip(dur 5.2~5.5s)이 `extract_six` 결정론적 실패**(대표 `7af0e302` read-only 6단계 진단으로 확인: input-seek `-ss t -i`가 sparse-decodable clip(13 vs 32 frames) 이른 timestamp 에서 rc=234 'Conversion failed!'; duration mismatch·clip-short 아님, keyframe 수 미측정. `8c472464`·`cfa27599` 는 3/3 동일 실패 확인·동일 저수준 원인 추정). 3 clip×2 cache×3 repeat = **18 measured key 영구 미완성** → 동결 계약 §9 PASS(전체 key 완전성) 원리적 불가 → `S1R_PASS` 불가 확정, 추가 계산 중단. **CROI mps cold capacity 1435.36 clips/h 는 160 게이트를 크게 상회하나 완전성 미충족으로 참고값(informative-only)**이며 adoption·gap 필드 배치 결정 근거로 쓰지 않는다. 안전 전부 통과(temp0·DB/R2 write0·VLM0·LaunchAgent 조작0·production job 지연0·nightly `19a1fe5`/gate `f182ea4b` 불변). 독립 재계산 harness 정확 일치. **재개 조건:** A6 baseline 을 완전성 계약에 둘지 재설계(새 spec+TEST-SHEET) — 결정론 실패 clip 명시 제외 / `extract_six` output-seek 견고화(production 변경=별도 승인) / sparse-decodable clip 표본 분리. 상세: [`recovery REPORT`](../../../experiments/python-evidence-s1-recovery/REPORT.md) · [`완료 보고서`](../../handoff-prompts/2026-07-17-python-evidence-s1-recovery-report.md).
   - **[2026-07-17] S1R2 = `S1R2_PASS_CROI_THROUGHPUT`(S2 allowed — 계획 작성만).** S1R HOLD 의 근본 = 비교용 A6 를 CROI 채택 게이트에 결합한 것(A6 는 CROI production path 선행 의존성 아님). S1R2 는 질문을 **CROI/MPS/cold 단독 처리량 하나로 좁혀** 고정 profile `croi-mps-cold`(A6 없어 FFmpeg not_required)로 새 raw 사전등록·측정. 한 안전창에서 **warmup32+measured96=128 완주, 96/96 complete**(missing/unexpected/dup 0, error 0). CROI cold **p95 2.550s / capacity 1,411.69 clips/h = gate 160의 8.82배**(projected 4cam p95 80s 대비 17.65×). RSS 1.10GiB(<4)·temp 16MB(<2). harness↔독립 재계산 counts·percentile 정확 일치(raw sha `48ffe6e9`, previous S1/S1R raw 미사용). 운영 안전 전부 통과(production HEAD nightly `19a1fe56`/gate `f182ea4b`/lab `df7811c1` 불변·워커 exit0·router-features PID 무재시작·err Δ0·temp0·DB/R2 write0·VLM0·LaunchAgent/plist/env 미수정). **결론: CROI(= B 경로의 gate sampling + detector + bbox ROI dense flow)는 Mac mini MPS cold 에서 처리량·운영 안전상 production 후보로 유효.** gap 필드 배치는 여전히 미결정이나(dwell/ROI series 의 detector 의존 여부), CROI path 자체의 처리량 안전은 확보. PASS 는 production selector 적용·자동 제외·행동 분류 승인이 **아니며** S2 raw-evidence shadow **구현계획 작성만** 허용한다. 상세: [`S1R2 REPORT`](../../../experiments/python-evidence-s1r2-croi/REPORT.md) · [`완료 보고서`](../../handoff-prompts/2026-07-17-python-evidence-s1r2-croi-report.md).
