...
| 단계 | 내용 | 산출 | 게이트 |
|---|---|---|---|
| **S0 정합화·검증** | live runtime SOT 정정(§1), Evidence contract(§7) 문서 확정, 기존 evidence가 production 카메라에 채워지는지 실측(현재 activity-worker는 테스트 카메라만) | 이 문서 + 실측 리포트 | 없음(문서·read-only) |
...
1. **production 카메라의 gate evidence 채움 여부** — 호스트 오배치 문제는 해결됐고 Mac mini producer evidence 88건을 실측했다. activity-worker는 Mac mini에서 테스트 카메라 3대를 매시간 처리한다. 다만 전체 카메라·날짜별 `clip_prelabels` 채움률과 selector 입력 시점의 coverage는 아직 산출하지 않았으므로 S0 read-only 감사 항목으로 유지한다. 정규 candidate 경로는 `enrich_prepool`로 on-demand 채우지만, "선택 전 전체 풀"이 아니라 "적재된 풀"만이다. production clip에 clip_prelabels가 얼마나 채워졌는지 실측 필요(S0).
