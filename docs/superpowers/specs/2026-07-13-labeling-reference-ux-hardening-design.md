...
> 상태: **production 적용·owner 보정·5개 seed·`tutorial-v1` 활성화 완료 (2026-07-14)**.
> 남음: 활동 중 라벨러 2명 중 지정한 1명의 tutorial 5/5 E2E·본 큐 진입·첫 본작업 5개 확인.
> 검증: web 245 tests + tsc + Vercel production build + Python 334 통과. DB rollback probe와 production UI 확인 완료.
> 구현·릴리스 범위 = §12 step 1~15·17 완료, step 16(실제 labeler E2E) 진행 대기.
>
> 연관 문서:
...
| 3 | `e679f8ad` | drinking+wheel, target `glass` | ✅ owner 보정 UI로 실제 접촉면 `glass` 확정, revision 기록 |
| 4 | `d9346cbe` | hand feeding+licking+tool, context `human` | ✅ owner 보정 UI로 `human` 추가, revision 기록 |
| 5 | `8669a5ff` | drinking, VLM shedding incorrect | 변경 없음 |

3번 target은 owner가 영상을 다시 보고 물이 직접 보이지 않지만 유리 표면을 반복해 핥는 장면으로
판단해 `glass`로 확정했다. 제출 전 문구는 정답을 노출하지 않고 관찰된 접촉 대상을 고르게 하며,
제출 후 feedback에서 이 영상의 `glass` 근거와 wheel evidence 분리를 설명한다.
...
15. ✅ activation
16. 🚧 활동 중 labeler 2명 중 지정한 1명 5개 E2E·본 큐 진입·첫 본작업 5개
17. ✅ production 배포·운영 화면 확인
...
- seed·activation·production 운영 화면 확인이 완료되고, 실제 labeler E2E만 남는다.
