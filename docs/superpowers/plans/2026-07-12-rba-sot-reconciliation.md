...
- [ ] **Step 1:** 북극성 상태를 확정으로 바꾸고 2026-07-12 실행 원칙을 추가한다.
- [ ] **Step 2:** Track A 표에서 현재 모델 미확정, Gemini historical, Track B 품질 연구를 명시한다.
- [ ] **Step 3:** Evidence-First와 local router 섹션에 유효성 감사 이후의 채택/중단 경계를 반영한다.
- [ ] **Step 4:** Phase 1의 v3.5 production 고정 문구를 frozen API baseline 선정으로 교체한다.
...
- [ ] **Step 1:** `next-session.md` 최상단에 2026-07-12 결정 블록을 추가해 이전 7월 10일 착수점을 supersede한다.
- [ ] **Step 2:** 기존 72/203/v1/v1.1 데이터는 EDA 전용이고 다음 작업은 baseline 계약 동결임을 명시한다.
- [ ] **Step 3:** specs 인덱스의 v3.5 항목을 historical로 바꾸고 `router-cost-v2` 실행 전 상태를 등록한다.
- [ ] **Step 4:** 시험지에서 30 P0 pilot과 150 P0 adoption을 구분하고 비용 tracker, retry, prompt caching 계약을 추가한다.
...
- [ ] **Step 1:** README의 Track A와 로드맵을 현재 모델 미확정·baseline 계약 대기로 바꾼다.
- [ ] **Step 2:** AGENTS와 CLAUDE의 Track A 정의를 역할 중심으로 바꾸고 테스트 수를 최신화한다.
- [ ] **Step 3:** FEATURES와 GLOSSARY의 Gemini production 표현을 historical baseline으로 교체한다.
- [ ] **Step 4:** 모든 진입 문서에서 최신 감사와 시험지 링크가 도달 가능한지 확인한다.
...
- [ ] **Step 1:** `rg`로 금지된 활성 표현과 placeholder를 검사한다.
- [ ] **Step 2:** 로컬 Markdown 링크 대상이 존재하는지 검사한다.
- [ ] **Step 3:** `git diff --check`를 실행해 공백 오류가 없음을 확인한다.
- [ ] **Step 4:** `uv run pytest`를 실행해 334개 전체 테스트가 통과하는지 확인한다.
- [ ] **Step 5:** `cd web && npx tsc --noEmit`을 실행해 TypeScript 오류가 없음을 확인한다.
