...
- [x] **Step 1: dependency를 lockfile에 추가한다**
...
- [x] **Step 2: 허용·제한·오류의 failing test를 작성한다**
...
- [x] **Step 4: 최소 adapter와 async handler를 구현한다**
...
- [x] **Step 5: focused test를 GREEN으로 만든다**
...
- [x] **Step 1: production flag matrix failing test를 작성한다**
...
- [x] **Step 3: provider/UI의 공통 enable 판정을 구현한다**
...
- [x] **Step 4: focused test를 GREEN으로 만든다**
...
- [x] **Step 2: secret/경계 정적 감사를 실행한다**
...
- [x] **Step 3: Codex CLI 독립 read-only review를 실행한다**
...
- [x] **Step 1: 현재 production과 worker identity를 read-only로 재확인한다**
...
- [x] **Step 2: WAF rule을 publish하고 active config를 read-back한다**
...
- [x] **Step 3: production server env를 비밀 노출 없이 준비한다**
...
- [x] **Step 4: flag-off production을 배포해 fail-closed를 확인한다**
...
- [x] **Step 5: flag를 추가하고 public deployment를 만든다**
...
- [x] **Step 7: WAF negative canary를 실행한다**
...
- [x] **Step 8: 실제 rollback과 복귀를 검증한다**
...
- [x] **Step 9: evidence와 상태를 문서화하고 commit/push한다**
