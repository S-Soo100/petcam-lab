...
**Files:**
- Create on Mac mini private artifact: `/Users/baek-end/private-rba/yolo26n-v22-candidates/attempt-20260810-owner-v1/`
...
ssh baek-end@baeg-endeuui-Macmini.local \
  'mkdir -p /Users/baek-end/private-rba/yolo26n-v22-candidates/attempt-20260810-owner-v1/code && tar -x -C /Users/baek-end/private-rba/yolo26n-v22-candidates/attempt-20260810-owner-v1/code'
```
...
PYTHONPATH="$RUN/code" "$VENV/bin/python" \
  ...
  --probe-max-frames-per-night 24 \
  --seed owner-v2.2
```

Expected: production-purpose eligible 수, exclusion 수, selected/downloaded/missing 수가 비밀 없는
집계로 출력되고 DB/R2 write는 0이다.
...
Expected: exact 320장 또는 bucket별 shortage가 fail-closed로 보고된다. shortage를 다른 bucket으로
채우지 않는다.
...
```bash
RUN=/Users/baek-end/private-rba/yolo26n-v22-candidates/attempt-20260810-owner-v1
python3 -c 'import json,pathlib; p=pathlib.Path("/Users/baek-end/private-rba/yolo26n-v22-candidates/attempt-20260810-owner-v1/candidate-manifest.private.json"); d=json.loads(p.read_text()); assert d["prediction_boxes_exposed_to_reviewer"] is False; assert d["db_write_count"] == 0; assert d["r2_write_count"] == 0; print(d["status"], d["review_frame_count"], d["bucket_counts"], d["camera_night_count"])'
```
