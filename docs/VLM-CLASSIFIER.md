...
- **현재 성능** — **85.5%** top-1 (159건 평가, feeding-merged UX 매핑 후)
...
### 2.3 결과 (2026-04-30)
- **85.5%** top-1 (159건 평가, feeding-merged) → **가설 성립** ✅
...
| 셋 | 클립 수 | 위치 | 비고 |
|---|---|---|---|
| **v3.5 SOT** | **159** | `web/eval/v35/v3.5-zeroshot.jsonl` (또는 `/tmp/`) | 정확도 85.5%(merged) lock-in baseline |
...
baseline (v3.5 85.5%) 깨기 시도 6번 모두 floor 미달. **다른 에이전트가 반복하지 말 것.**
...
- raw 9-class 평가 vs UI 8-class 평가 둘 다 추적 (raw 81.8% / merged 85.5%)
...
5. baseline (85.5%) 비교 → 5-카테고리 분석 (held-correct/recovered/broken/still-wrong)
...
- `~/.claude/projects/-Users-baek-petcam-lab/memory/`
  - `project_vlm_v35_baseline_lock.md` — 85.5% / 85.7% floor
...
**Q. 왜 85.5%를 production이라 하나? 더 높일 수 있지 않나?**
...
**Q. fine-tune 안 하나?**
A. HITL 답 충분히 누적된 후 별도 spec. 현재는 zero-shot으로 충분 (85.5%).
