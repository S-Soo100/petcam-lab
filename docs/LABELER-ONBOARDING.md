...
> 운영 상태(2026-07-13): 가입·승인·날짜 선택은 preview 검증 완료, production 승격 전.
> 대화형 튜토리얼은 설계 완료, 구현 전. production 배포가 끝나기 전에는 팀원에게
> 가입 링크를 배포하지 않는다.
...
5. 각 영상은 `Blind GT → VLM 검수 → 기준 답·차이·해설 확인` 순서로 진행한다.
