...
- 검증: Python 334, Web 60 tests, TypeScript 통과. clean worktree에서 Vercel preview
  `dpl_D45VbxNBBFBzsXYoLL7DeD939PDX`를 빌드해 `READY`를 확인했다. 보호 우회 HTTP 검증에서
  `/labeling/signup`은 200, 비인증 `/api/labeling-access`와 `/api/labeling-v2/queue`는 401이었다.
  preview는 Vercel Deployment Protection 대상이라 실제 가입→owner 승인→큐 진입 브라우저 E2E는
  production 승격 후 실제 팀원 계정으로 확인한다.

상세 [가입·승인·날짜 설계](superpowers/specs/2026-07-13-labeler-signup-date-controls-design.md).

**대화형 튜토리얼(2026-07-13, 코드 구현 완료·콘텐츠/배포 전):** 승인된 신규 라벨러는 owner가
...
- **하드닝(후속 마이그레이션 `_hardening.sql`)**: active/archived lesson 의 clip/reference/
  prediction/feedback 변경·삭제를 DB trigger 로 차단. seed 는 draft set 전용 + owner
  session 의 VLM verdict/`completion_reason=vlm_reviewed`·비어있지 않은 feedback 검사.
  activation 은 draft 전용 + 5 lesson verdict/feedback 완전성. Router Review API 도
  production 게이트 적용, 미완료 labeler 에게 work 메뉴 숨김. reset/waive 는 대상이 실제
  labeler 인지 검증(404). 레거시 무인증 `/api/label` 은 owner-only 로 잠금.
- 검증: web 115 tests(비교·게이트·gt/vlm/acknowledge/media/owner 라우트·`/api/label` 회귀),
  `next build`·tsc·Python 334 통과. active lesson 불변은 DB 롤백 검증(ALL_PASS_ROLLBACK).
  **미완**: 마이그레이션 2개 적용됨, owner 5개 seed·activation·실계정 E2E·production 배포는 남음.
