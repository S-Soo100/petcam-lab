...
**화면:** `/labeling/mine`·`/labeling/all`(필터: 라벨 안 됨/됨, 하이라이트 O/X/대기, 카메라) → `/labeling/v4/[clipId]`(영상+GME 오버레이+`1차 판정: O — 근거`+`O 확정/X 확정`, 다르면 사유 칩, 확정 뒤 같은 카메라의 다음 안 된 영상으로 서버측 cursor 이동) · owner `/labeling/owner`(라벨 안 됨·오늘/7일 확정·회원별·카메라별) · `/labeling/team` 카메라 배정 패널.
