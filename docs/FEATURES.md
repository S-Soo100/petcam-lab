...
Stage A ~ D5 + 클라우드 마이그레이션 (`cloud-migration-roadmap.md`) 까지 거치면서 13개 기능 레이어가 쌓였다. 아래 순서는 데이터 흐름 순 (캡처 → 저장 → R2 → 조회 → 라벨링 → 인증 → VLM → 라벨링 웹 → 배포 → QA).
...
3. **VLM 워커** (`backend.vlm_worker_main`) — fly.io `petcam-vlm-worker` always-on. §10.
...
**어디**
- [backend/r2_storage.py](../backend/r2_storage.py) — `presign_get_url(key, ttl=3600)` + `put_object` 헬퍼
...
**관련 스펙:** [feature-clip-r2-storage.md](../specs/feature-clip-r2-storage.md), [cloud-migration-roadmap.md](../specs/cloud-migration-roadmap.md)
...
## 10. VLM 자동 라벨링 (별도 워커, fly.io always-on)
...
- **DB-as-message-bus** — Redis 같은 외부 큐 없이 `behavior_logs` 테이블 자체를 큐로 사용:
  ...
  - VLM 워커가 `status='pending'` 행을 폴링 → R2 signed URL 로 mp4 다운로드 → Gemini 2.0 Flash 호출 → `action`/`confidence`/`reasoning` UPDATE + `status='done'`.
...
**모델 / 비용**
- 디폴트 — Gemini 2.0 Flash (저비용·결정론 우선, `temperature=0.1`)
- 9 raw ActionType 중 1개 + confidence 0~1 + reasoning. 클립당 ~$0.001.
