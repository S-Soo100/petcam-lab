...
**⭐ 대표 tier(2026-09-10):** owner "하이라이트가 너무 많다"(주 카메라 하루 O 중앙값 28·최대 95) → 임계값 상향(사람 O 61% 손실)·시간당 top-5(66% 잔존) 탈락, **O/X 는 후보 자격으로 두고 조회 시 계산되는 예산 레이어**를 얹음. `fn_highlight_featured`: 하루(20:00→다음날 20:00 KST) 안 카메라별 30분 에피소드 묶기 → 사건 점수(✨ 최우선 > 사람 O > activity 합) top-3 의 대표 클립 = `featured`, 나머지 O = `candidate`. 저장 없음(라벨 아님·순위) — X/✨ 를 바꾸면 다음 조회에 순위가 바뀐다. 앱 `GET /highlights/featured`(대표만 기본, `tier=all` 후보), 라벨링 웹 카드 `⭐ 대표 n위`/`후보` 배지 + `⭐ 대표만` 칩(최근 7일) + 상세 한 줄. 규칙 params·표본·2.6.1 절차 불변. 스펙 `specs/feature-highlight-featured-tier.md`, 런북 §6.y. **재생 시작점(2026-09-12):** 대표·목록 함수 출력 `first_moving_sec` + petcam-api item `play_from_sec`(첫 움직임 − 1.5초, 3초 이후만) — 앱이 움직임 직전부터 재생, 숫자 표시 없음(핸드오프 `2026-09-12-flutter-play-from.md`).
