...
**v2 방향:** 이 웹을 단순 검수 화면에서 사람 GT·VLM 검수·metadata 생산 도구로 발전시킨다. 한 화면에서 prediction을 숨긴 사람 GT를 먼저 확정하고, 그 뒤 VLM verdict와 오류 유형을 저장한다. 필수 GT는 visibility·대표/복수 관찰 행동·구간·target·확신도·품질/환경 tag이며 camera/animal/enclosure/provenance는 시스템 상속, bbox/ROI는 조건부 고급 모드다. 상세 [라벨링 웹 v2 설계](superpowers/specs/2026-07-12-labeling-web-v2-design.md) · [RBA Data Engine v1](../specs/feature-rba-data-engine-v1.md).
