...
**RBA (Reptile Behavior Analysis)** 는 이 제품의 핵심 AI 분석 기술명이다. 밤사이 파충류 펫캠 영상을 행동 타임라인과 케어 시그널로 바꾼다. Track A = Zero-shot VLM 운영 라벨링, Track B = SegmentVLM 정밀 분석/실험 트랙. 사업·관계도 설명은 [`docs/AI-VIDEO-ANALYSIS-STRATEGY.md`](AI-VIDEO-ANALYSIS-STRATEGY.md).
...
- **DB-as-message-bus** — Redis 같은 외부 큐 없이 `behavior_logs` 테이블 자체를 큐로 사용:
  ...
  - VLM 워커가 `status='pending'` 행을 폴링 → R2 signed URL 로 mp4 다운로드 → Gemini 2.5 Flash 호출 → `action`/`confidence`/`reasoning` UPDATE + `status='done'`.
...
**모델 / 비용**
- 디폴트 — Gemini 2.5 Flash (저비용·결정론 우선, `temperature=0.1`)
