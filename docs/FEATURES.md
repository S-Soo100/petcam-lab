...
- 검증: web 125 tests(비교·게이트·gt/vlm/acknowledge/media/owner/router-review/team-progress·
  `/api/label` 회귀), `next build`·tsc·Python 334 통과. active lesson 불변은 DB 롤백 검증
  (move/position/title/reference/delete 5차단 ALL_PASS_ROLLBACK).
  **미완**: 마이그레이션 3개 적용됨, owner 5개 seed·activation·실계정 E2E·production 배포는 남음.

**기준 GT·튜토리얼 UX 하드닝(2026-07-13, 코드+DB 적용 완료·seed/배포 전):** owner 가 튜토리얼
...
- 검증: web 155 tests(collectGroundTruthIssues 9규칙·allowedTargets·diff·튜토리얼 absent 비교·
  ...
  **DB 롤백 probe 통과**(원자 보정·initial_gt 불변·reviewer/reason 차단·seed mismatch 차단).
  `npm run build` 는 사용자 터미널에서 실행 필요(레포 훅이 세션 내 build 차단).
  **미완(별도 승인)**: owner e679/d934 실보정·tutorial seed·activation·production 배포.
