...
### 6.x 행동 표시 4종은 규칙과 무관 (2026-09-11 확장)

`motion_clip_behavior_flags`(2026-09-09, **2026-09-11 `kind` 컬럼 추가·PK `(clip_id, kind)`**)는 하이라이트 O/X·유지율·`change_reason` 어디에도 안 들어간다. 종류 = `meaningful`(✨ 의미있는 행동: 물·허물·밥 등) · `wheel`(🎡 쳇바퀴 — "제자리 활동" 정답 세트) · `fall`(⚠️ 추락 — 수집용, 대표 선정 무관) · `closeup`(📸 예쁘게 나옴 — 자동 클로즈업 검증용). 영상당 종류별 1개, 종류끼리 독립, **미표시 = 미확인**(음성 아님). 옛 ✨ 행은 전부 `meaningful`. RPC: `fn_get_motion_clip_behavior_flags(clip)`(4행) · `fn_set_motion_clip_behavior_flag(clip, user, is_owner, kind, flagged)` · 배치 `fn_get_motion_clip_behavior_kinds(uuid[])`; 옛 4-인자 set·단일 get 은 `meaningful` 위임. 대표 선정(v0.1.1)에선 `meaningful|wheel|closeup` 중 하나라도 있으면 그 사건이 1위 승격(추락 제외). 필터: 목록 `?behavior_flag=yes|meaningful|wheel|fall|closeup`, 단축키 F/W/D/P. SQL `select clip_id, kind, flagged_by, flagged_at from public.motion_clip_behavior_flags order by flagged_at desc`.

**🔎 의심 서명 칩(2026-09-11):** 정지 게코 bbox 흔들림 서명(최장 연속 <2s & moving 구간 ≥30)인 현재-O 클립을 표본 `jitter-2026-09` 로 등록(`scripts/build_jitter_regression_sample.py --register`, owner 승인). 목록 칩으로 팀원이 먼저 검수(정지 오탐이면 X + 오검출) + 2.6.1 전환 전후 같은 영상으로 회귀 비교. 규칙 자동 가드는 넣지 않는다(사람 O 에도 같은 서명 존재, ChatGPT 검토 2026-09-11). 배경: `docs/highlight-motion-gaps-options.md`.

### 6.y ⭐ 대표 tier 는 규칙이 아니라 예산 레이어 (2026-09-10, v0.1 2026-09-11)

`fn_highlight_featured(카메라[], from, to, 계약 3, top_n=NULL, gap_sec=600, day_start_hour=20, tz='Asia/Seoul', hour_cap=3, day_cap=15)` 는 (**v0.1.1 2026-09-11: 하루·카메라당 대표 예산 `day_cap` 15 — owner 임시값, 일주일 뒤 조정. 시간당 상한에 걸린 사건은 예산을 안 먹음. 출력에 `behavior_kinds`**; **v0.1.2 2026-09-12: 출력에 `first_moving_sec`** — 목록 `fn_list_labeling_v4_clips` 14/13/12-인자에도 같은 컬럼, migration `2026-09-12_highlight_first_moving.sql`. 앱 재생 시작점 `play_from_sec` 은 petcam-api 가 계산(리드 1.5초·첫 움직임 3초 이후만, `PLAY_FROM_*`). 옛 wrapper 가 `SELECT *` 라 목록 반환 컬럼을 늘릴 땐 세 시그니처를 함께 DROP+CREATE 해야 한다) 현재 O(사람 확정 우선) 클립을 하루(20:00 KST 경계)·카메라별 **10분** 에피소드로 묶고 사건 점수(✨ > 사람 O > activity 합) 순으로 대표 클립에 `tier='featured'` 를 붙인다. **v0.1(owner 2026-09-11): 하루 상한 없음(top_n NULL), 대신 같은 시간대(대표 클립의 KST 시) 안 최대 hour_cap=3.** 실측 주 카메라 하룻밤 11개(최대 16). 옛 조합(30분·하루 3)은 인자로 여전히 호출 가능(`migrations/2026-09-11_highlight_featured_hour_cap.sql`, DROP+CREATE 로 교체). **저장하지 않는다** — X/✨ 를 바꾸면 다음 조회에 순위가 바뀐다. 소비처: petcam-api `GET /highlights/featured`(대표만 기본, `tier=all` 후보 포함), 라벨링 웹 목록 배지·`⭐ 대표만` 칩·상세 한 줄. 규칙 params·유지율·표본과 무관. 기본값을 바꾸려면 DB DEFAULT·petcam-api 상수(`FEATURED_*`)·`web/src/lib/labelingV4.ts` 상수 세 곳을 같이. 기간은 31일 상한(범위 밖 스캔 방지). 실측: `uv run python scripts/report_highlight_featured.py --days 7 --contract <algo> <detector>` (콜드 ≤ 3초 게이트, 카메라×하루 대표 ≤ top_n). 스펙 `specs/feature-highlight-featured-tier.md`, 결정 로그 2026-09-10.
