...
### 2026-07-23 — P4 Cam 1 쳇바퀴 에피소드 중복 묶음 (판정자: Codex + owner 방향 승인)

맥락: owner 라벨링 중 P4 Cam 1 쳇바퀴 영상이 반복해서 나타나는 부담을 확인했다. production SELECT-only
감사에서 확인된 wheel GT 24개는 10분 경계 기준 4개 에피소드에 집중됐지만, 같은 시간 범위의 전체
54개 중 30개는 다른 GT 또는 미라벨이었다. generic Python Evidence도 wheel/non-wheel 분포가 겹쳐
시간 또는 단일 수치 임계값만으로는 안전하게 묶을 수 없다. 설계 정본:
[`2026-07-23-wheel-episode-dedup-design.md`](superpowers/specs/2026-07-23-wheel-episode-dedup-design.md).

| 제안 | G1 SOT | G2 효과 | G3 측정 | G4 계획 | 판정 | 근거 |
|---|---|---|---|---|---|---|
| P4 Cam 1 고정 wheel ROI + 프레임 유사도 + 시간 경계로 같은 에피소드 후보를 묶고 대표 2~3개를 우선 표시 | ✓ | ✓ | ✓ | ✓ | **shadow 설계 승인** | curated event UX와 사람 GT 생산 효율에 부합한다. 확인 표본에서는 우선 검토량 24→8~12의 가능성이 있지만 adoption 근거로 재사용하지 않는다. fresh camera-night≥3·membership≥100에서 mixed-action false merge=0, interaction 대표 보존=100%, known wheel workload 감소≥50%를 사전등록한다. Phase 1은 read-only artifact뿐이며 GT·triage·R2·VLM·활동시간을 바꾸지 않는다. |
