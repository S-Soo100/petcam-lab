...
## 6.8 Detector-Independent Local Router — gate v2 reject 이후의 우회 연구

> 결론: **local router v0는 detector 없이 시작하되, skip/auto-label은 금지한다.**
...
그래서 다음 local 연구는 detector를 필수 입력으로 가정하지 않는다. 새 트랙은 **Detector-Independent Local Router**다.
