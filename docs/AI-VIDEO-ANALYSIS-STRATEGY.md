...
| 현재 상태 | 역할 유지·production 모델 미확정 | 품질 연구·production 미적용 |
...
**보강 — 병목은 시간축만이 아니다: 공간 해상도도 벽이고, 실패는 "조용하다".**
위 3층은 "정보가 시간축에 있어서"에 방점을 두지만, 실측상 미세행동 병목은 **두 겹**이다: ① 시간축(움직임 시퀀스), ② **공간 해상도**(혀-물 접촉이 정지 프레임에 있어도 혀가 임계 픽셀보다 작으면 판독 불가). 입력표현·프롬프트·모델·ROI crop 4개 레버를 모두 소진해도 천장이 유지된 근거가 이것이다(`experiments/roi-crop-center`, decision=close).

**왜 구조적 한계인가:** VLM의 이미지 인식은 OCR이 아니다. 이미지를 픽셀이 아니라 **패치**(Anthropic 공식 vision 문서: 28×28px = 1 visual token) 단위 임베딩으로 뭉뚱그리므로, 글자·특징 단위의 신뢰도가 존재하지 않는다. 특징이 인코더 해상도 아래로 내려가면 모델은 "불확실"을 낼 수 없고 **언어 prior로 그럴듯한 오답을 지어낸다(confabulation)** — 에러가 아니라 확신에 찬 틀림. 외부 오픈소스 벤치마크(pxpipe, 2026-07)가 같은 패턴을 정량화했다: 판독 정확도는 특징의 픽셀 면적에 단조 증가하고(5×8px 셀 10% → 20×32px 셀 100%), 임계 밑 오답은 "빈칸"이 아니라 "한 끗 어긋난 그럴듯한 값"으로 나타난다. **게코의 혀-물 접촉은 그들의 미세 글자와 같은 신호다.**

**함의:** ① 3층(시간축·HITL)은 대체가 아니라 **필수** — 정지 프레임 VLM의 구조적 한계다. ② 실패가 조용한 confabulation이라 모델 자체 신뢰도로 거를 수 없다 → **HITL·검증 레이어가 안전장치**다. ③ 공간 해상도가 실측 병목인 만큼 **자체 HW의 고해상·비흔들림 캡처**가 미세행동 정확도의 직접 레버다(현 평가셋은 대부분 폰 저해상·흔들림이라 이 레버가 미측정 — 2026-07-06 확인).
...
## 6.6 캡처 민감도 vs 분석 비용 — 왜 gate(사전 검출)인가

> 2026-07-07 백로그 백필 중 실측으로 규명. 결론: **캡처를 줄이면 안 되고, 분석 비용을 gate 로 줄여야 한다.**

### 문제 — 모션 트리거 과민으로 clip 폭발

모션 트리거 캡처가 과민해서, 밤(20~06시) 한 카메라에만 수백 개 clip 이 쌓인다(실측: 07-03밤 A 카메라 374개 = 약 1.6분에 1개, 거의 연속 생성). 이 clip 들은 `motion_score` 가 **전부 ≤0.1**(큰 움직임 아님)이고, claude 분류 결과 **약 30% 가 unseen**(게코가 아예 안 보임) = 노이즈·미동·그림자로 헛트리거된 것이다. 전수 rba 하면 이 저정보 clip 에 claude 비용이 그대로 나간다(4밤 1285개 전수 = claude 약 1285회 = 구독 한도 폭발).

### 왜 "모션 임계값 상향"은 답이 아닌가 (핵심 함정)

직관적으로 "`motion_score` 임계값을 올려 작은 clip 을 버리자"가 떠오르지만 **틀렸다**. 게코의 가장 중요한 케어 행동인 eating_paste·drinking 은 **몸은 정지, 혀만 미세하게 움직이는** 행동이라 `motion_score` 가 거의 0 이다. 임계값을 올리면 노이즈와 함께 **이 미세 케어행동도 통째로 잘린다**.

즉 `motion_score`(움직임 크기) 하나로는 **"노이즈"와 "미세 케어행동"을 구분할 수 없다** — 둘 다 저값이기 때문이다. 크기로 자르는 방식(임계값·고정 시간간격 샘플링)은 모두 미세행동을 놓칠 위험이 있다.

### 결정 — 캡처는 민감하게 유지, 비용은 분석 단계에서 gate 로

| 단계 | 방침 | 이유 |
|---|---|---|
| **캡처** | 지금처럼 **민감하게 유지** | 혀만 움직이는 식사/음수를 놓치지 않으려면 작은 움직임도 잡아야 한다. clip 수가 많은 건 그 대가(스토리지는 30초 clip 이라 저렴, 진짜 비용은 claude 분석). |
| **분석(rba)** | **gate(detector)로 unseen 사전 필터** | 캡처는 다 잡되, 분석 전에 "게코 없는 clip"을 걸러 claude 호출을 아낀다. |

핵심 원칙: **"움직임 크기"가 아니라 "게코 존재/위치"로 거른다.** 게코가 밥그릇 앞에 정지해 있어도(혀만 움직여도) 프레임에 게코가 있으면 분석 대상이며, 이걸 판단하는 것이 gate(`gecko-vision-gate`)의 detector 다.

### gate 도입 근거와 검증 조건

- **ROI 근거(실측)**: 백로그 노출률 = 게코 보임 약 70% / unseen 약 30%. gate 가 unseen 을 걸러내면 claude 분석 비용 **약 30% 절감**. 시간대 편차 있음(22시대 unseen 0%, 20~21시 약 35%).
- **도입 전 필수 검증**: detector 의 **게코 검출 recall**. detector 가 "게코 있는데 없다"고 오판(false negative)하면, 아끼려던 unseen 이 아니라 **진짜 미세 케어행동을 버리는** 참사가 된다. OWLv2 가 47.5% 에서 검출 실패한 전례가 있으므로, gate 배포 전 이 백로그의 claude 판정(unseen/moving)과 detector 판정을 대조해 recall 을 확인한다.

관련: §6.5 YOLO Evidence 레이어(같은 detector 기반 evidence 계열), 자매 레포 `gecko-vision-gate`.

## 6.7 Evidence-First Cascade — 새 전략 (2026-07-09)

> 결론: **preprocessor-first는 채택, OpenCV-only auto-label은 보류.**

Evidence-First Cascade는 SegmentVLM과 다른 전략이다. SegmentVLM은 중요한 clip을 더 잘게 쪼개 VLM이 정밀 판독하게 하는 Track B 전략이고, Evidence-First Cascade는 **VLM 호출 전에 Python/OpenCV/detector가 싼 증거를 만들고 라우팅하는 비용 절감 전략**이다.

실험 SOT: [`../specs/experiment-rba-evidence-first-cascade.md`](../specs/experiment-rba-evidence-first-cascade.md), 결과: [`../experiments/rba-evidence-first-cascade/REPORT.md`](../experiments/rba-evidence-first-cascade/REPORT.md).

Full `dataset-203` 197건 결과:

- Python/OpenCV preprocessor가 영상 디코딩·프레임 선택을 맡고 VLM이 adaptive frames만 보는 경로는, 120k tokens/clip급 direct-video 기준 대비 **83.6% 토큰 절감**을 유지한다(`19,730 tokens/clip`, accuracy drop 0.00pp vs fallback baseline).
- OpenCV motion feature만으로 `moving`을 자동라벨링하는 경로는 실패했다. conservative rule 기준 full 197에서 non-VLM 자동처리 6.1%, false auto-label 75.0%였다. drinking/hand_feeding/shedding도 큰 motion으로 잡혀서 안전하지 않다.

83.6%는 동일 개발 데이터에서 입력 토큰 경로를 비교한 **전처리 엔지니어링 근거**다. 독립 holdout에서 새 production 모델의 행동 품질을 입증한 수치가 아니므로, adaptive-frame 경로는 유지하되 production 정확도 주장에는 사용하지 않는다.
...
즉시 가능:
video direct input
→ Python/OpenCV adaptive frame preprocessor
→ VLM semantic judge
...
다음 확장은 detector evidence가 붙은 뒤 재검증한다. 자동라벨/skip은 `motion_score`가 아니라 **gecko presence + object/ROI evidence**를 기준으로 해야 한다.

## 6.8 Detector-Independent Local Router — 중단 / exploratory

> **2026-07-12 유효성 감사 판정:** metadata/review 인프라는 유지한다. v0/v1/v2와 care-guard v1/v1.1은 exploratory failure evidence이며 `invalid-for-adoption`, 비용 절감은 `not-measured`, 현재 production eligibility는 `rejected`다.
...
- 따라서 현 detector로 unseen skip/gate를 production에 켜지 않는다

이 구조로 detector 없는 우회 가능성을 탐색했지만, 기존 데이터 재사용과 사후 threshold 수정 때문에 독립 성능 근거가 되지 못했다. 아래 구조는 당시 연구 대상을 설명하는 historical 설계다.

```text
OpenCV / metadata / 운영 context
→ local text LLM router
→ cloud_now / cloud_later / activity_only / review_candidate
→ cloud VLM fallback
```

중요한 경계:

- local router는 행동 라벨을 확정하지 않는다.
- v0에서는 `skip`과 `auto_moving`을 금지한다.
...
| 평균 mp4 크기 | 3MB |
| 영상 보관 기간 | 30일 |
| Track A 비용 | 모델·입력 동결 후 실측 |
| Track B 비용 | 모델·입력·fallback 계약 동결 후 실측 |
| Track B 실행률 | 전체 clip의 5~15% |

과거 Gemini planning reference(현재 계약 산정에 사용 금지):
...
|---|---:|---|---|---|---:|---|
| Phase 0 | 1~10명 | 내부 실험 | 저비용 API baseline 후보 동결 | 샘플 실험 | 실측 전 | RBA 구조·비용 계약 검증 |
...
최적화 후 목표:
Track A를 저비용 API/Batch/adaptive-frame 조합으로 clip당 $0.0003~$0.0006
...
- 독립 future holdout에서 동결한 품질·P0 recall gate를 통과한다.
...
Track B trigger:
- evidence reliability가 낮거나 동결된 라우팅 규칙이 abstain한 clip.
...
- model routing: 검증된 저비용 API / batch / local worker 후보 선택.
- dead-letter queue: 실패 clip 격리와 재처리.
- observability: cost/user, cost/clip, failure rate, p95 latency, fallback rate.

비용 전략:
- Track A는 동결된 저비용 API와 batch 후보를 같은 future holdout에서 평가한다.
...
RBA는 모든 영상을 비싸게 분석하는 시스템이 아니다. Track A 역할이 전체 clip을 저비용으로 넓게 보고, Track B가 중요한 후보만 깊게 본다. 그 결과 사용자는 밤사이 행동 요약을 받고, 회사는 HITL 검수 데이터를 축적한다.

1만 명 규모에서는 AI inference가 핵심 비용이므로, 검증된 저비용 API/Batch/local worker를 섞는 model routing과 selective fallback이 단위경제를 결정한다. 장기적으로는 자체 서버와 fine-tuning을 통해 외부 API 비용을 줄이고, 도마뱀 행동 데이터셋 자체를 회사의 방어 가능한 AI 자산으로 만든다.
