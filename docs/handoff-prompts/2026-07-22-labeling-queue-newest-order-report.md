...
## `LABELING_QUEUE_NEWEST_PRODUCTION_VERIFIED`
...
GREEN 이다. 이후 owner 승인으로 main fast-forward·Vercel production 배포·인증 owner smoke까지
완료했다. 배포 전 Stop Point 기록은 아래 이력으로 보존하며, 현재 상태는 §11이 정본이다. Evidence
GT Work Package B 는 별도 handoff로 B1만 시작한다.
...
## 11. Production 배포 closure (2026-07-22)

- **main 통합:** feature tip `d99716cbb53ee6b9ec19d1feb9df77ea65766a2a`를 `origin/main`에
  fast-forward push했다. non-FF·force push·migration은 없었다.
- **Vercel:** deployment `dpl_5Y96FSnurEM62mSLo3wfjPnE6bCY`가 `READY`, production alias
  `https://label.tera-ai.uk` 연결 완료. Vercel build도 compile·lint/type·static **19/19** 통과했다.
- **인증 owner smoke:** production owner 계정으로 `/labeling` 진입, 로그인 redirect 없음,
  화면 오류·console warn/error 0.
- **첫 카드 독립 대조:** 화면 첫 clip `7d3f668e-8087-4994-8ee5-510326fbb8f7`가 같은 owner·
  `has_motion=true`·`r2_key!=null`·본인 completed session 제외·triage pending/skipped 제외 조건으로
  service-role **SELECT-only** 재계산한 최신 eligible clip과 정확히 일치했다
  (`started_at=2026-07-06T20:59:07+00:00`).
- **2페이지 keyset smoke:** production API 1페이지 30건 + 2페이지 30건 모두 200, 두 페이지 id
  overlap **0**, 각 페이지와 경계가 `(started_at DESC, id DESC)` 단조 감소. UI에서도 `더보기` 후
  60개 카드가 유지됐고 경계는 `05:17:52 → 05:16:49 KST`로 감소했다.
- **데이터 변경:** smoke는 로그인·GET·SELECT만 수행했다. 격리·라벨·GT·VLM·DB write 0.

최종 판정은 `LABELING_QUEUE_NEWEST_PRODUCTION_VERIFIED`. 다음 단계는 별도 manifest로 Evidence B1
(production SELECT-only 가용성 진단)만 실행하고, 결과와 무관하게 B2 전에 멈춘다.
