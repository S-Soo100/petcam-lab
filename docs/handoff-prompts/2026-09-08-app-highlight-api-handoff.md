...
- 응답: `{"highlights":[item…], "count", "rule_version", "featured":{"top_n"(null=없음),"hour_cap","gap_sec","day_start_hour","time_zone","days"}}`. item = 기존 `/highlights` 항목 필드(`clip_id, camera_id, camera_name, started_at, duration_sec, media_ready, source, reason, rule_version`) + `tier`, `day_key`, `activity_sec`, `behavior_flagged`, `episode:{rank, hour_rank, clip_count, activity_sec, started_at, ended_at}`. 정렬 = (day_key 최신, 카메라, 사건 순위, 시각 최신). keyset 없음(하루·카메라당 대표 ≤ 시간대 수 × hour_cap, 실측 ≤ 16).
