...
**백엔드 컴포넌트 가동 상태 (2026-05-08 기준):**
- **API 서버 (production)** (`api.tera-ai.uk`) — ✅ **fly.io `petcam-api` always-on 가동.** 2026-05-08 DNS cutover 완료. nrt, shared-cpu-1x 256MB, `/health` 200, `min_machines_running = 1`. Cloudflare DNS A/AAAA → fly.io edge 직결 (Cloudflare Tunnel 폐기). Let's Encrypt E8 cert (만료 2026-08-06). 사용자 맥북 의존 0. **Flutter `BACKEND_URL` = `https://api.tera-ai.uk` 그대로 사용.**
...
```bash
# API 서버 production (fly.io 직결, 2026-05-08 cutover 완료)
...
**Flutter BACKEND_URL:** `https://api.tera-ai.uk` 그대로. DNS cutover 완료 후 fly.io always-on 직결이라 production 도메인 그대로 시뮬레이터에서 즉시 검증 가능. 별도 staging 패치 불필요.
