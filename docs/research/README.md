...
1. R1 runner·ledger·`researchctl` 구현과 synthetic adversarial gate를 B에 고정한다.
2. final record 전용 C를 검증해 구현 provenance를 닫는다.
3. 실제 Mac mini LaunchAgent 설치·재부팅·24시간 시험은 별도 P3 package로 진행한다.
...
| `r1-mac-mini-runtime-foundation` | 구현 검증 중 | SQLite+JSONL, fencing, production 양보, pull-only 관측을 synthetic no-op으로 구현 | B 전체 검증·독립 리뷰 → C final record |
...
- R1 운영 runbook: [`R1-RUNTIME-RUNBOOK.md`](R1-RUNTIME-RUNBOOK.md)
