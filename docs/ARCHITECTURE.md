...
└──────┬───────┘                                  │  ┌────────────────────────┐ │
       ...
       │ 【1】 Supabase JWT (Bearer)              │  │ cameras / camera_clips │ │
       │      HTTPS GET/POST/PATCH/DELETE         │  │ labels / labelers      │ │
       │      api.tera-ai.uk                      │  │ pets / behavior_logs   │ │
       ▼                                          │  └────────────────────────┘ │
┌──────────────────┐                              └─────────────────────────────┘
│ Cloudflare DNS   │                                  ▲                ▲
│ A/AAAA (DNS only)│                                  │ SELECT         │
└────────┬─────────┘                                  │ (service_role) │
         │ fly.io edge (nrt)                          │                │
         ▼                                            │                │
┌─────────────────────────────────────────────────────┼─┐              │
│ 프로세스 #1 — backend.main:app  (fly.io petcam-api)  │ │              │
│  ┌─────────────────────────────────────────────────┐│ │              │
│  │ routers/clips.py    /clips · /clips/{id}/...    ││ │              │
│  │ routers/cameras.py  /cameras · CRUD · probe     ││─┘              │
│  │ routers/labels.py   /clips/{id}/labels|inference││                │
│  │                     /labels/{queue|mine}         ││                │
│  │ routers/me.py       /me/is_labeler               ││                │
│  │ main.py             / · /health                  ││                │
│  └────────┬─────────────────────────┬──────────────┘│                │
└───────────┼─────────────────────────┼───────────────┘                │
            │ R2 signed URL 발급      │ JSON 응답                      │
            │ (boto3.generate_presigned_url, 1h TTL)                   │
            ▼                         ▼                                │
       ┌─────────────────┐    ┌──────────────┐                         │
       │ Cloudflare R2   │    │ Flutter app  │                         │
       │ (mp4/jpg)       │    │ (URL 수령)    │                         │
       └────────┬────────┘    └──────┬───────┘                         │
                ▲                    │                                 │
                │  PUT (capture)     │ 【2】 GET <signed URL>           │
                │  GET (signed URL)  │      (헤더 없이 R2 직접 GET —    │
                │                    │       byte 가 API 서버 안 거침) │
                │                    ▼                                 │
                │           ┌─────────────────┐                        │
                │           │  Flutter 영상   │                        │
                ...
                │           └─────────────────┘                        │
                │                                                      │
                │  GET (mp4 bytes — VLM 분석용)                        │
                │                                                      │
┌───────────────┴─────────────────────┐    ┌──────────────────────────────┐
│ 프로세스 #3 — backend.vlm_worker_main│    │ 라벨링 웹 (label.tera-ai.uk)  │
│ fly.io petcam-vlm-worker             │    │ Vercel Next.js                │
│ (always-on, nrt, shared-cpu-1x)     │    │  owner 흐름 → Supabase/R2 직결│
│  ┌────────────────────────────────┐ │    │  라벨러 큐 → BACKEND_URL      │
│  │ camera_clips polling (60s)      │ │    └──────────────────────────────┘
│  │   has_motion=true               │ │
│  │   r2_key NOT NULL               │ │       service_role
│  │   NOT EXISTS(behavior_logs vlm) │ │      INSERT behavior_logs
│  └────────┬───────────────────────┘ │ ────────────────────────►
│           ▼                         │       (action / confidence /
...
**Flutter ↔ R2 두 단계 흐름** — 그림 안의 【1】【2】 가 핵심. 영상 한 클립 재생 시:

1. **【1】 Flutter → API 서버 (api.tera-ai.uk)** — Bearer JWT 로 인증.
   `GET /clips/{id}/file/url` → JSON `{"url": "...?X-Amz-Signature=...", "ttl_sec": 3600, "type": "r2"}`.
   API 서버는 DB 권한 체크 + R2 signed URL 발급만. **영상 byte 안 받음**.
2. **【2】 Flutter → R2 (signed URL 직접 GET)** — 헤더 인증 불필요.
   signed URL 자체에 `X-Amz-Signature` 토큰 박혀있어 R2 가 1시간 동안만 GET 허용.
   영상 byte 는 R2 → Flutter 다이렉트 (Cloudflare edge 캐시 활용).

**왜 이렇게?** ① fly.io 256MB 머신이 영상 stream 부담 안 짊. ② Flutter 가 R2 키를 영원히 모름 (signed URL 만 1시간 유효). ③ JWT 검증·권한 체크는 API 서버 단일 게이트로 일원화. 자세한 결정 근거는 §8 표 + [feature-r2-storage-encoding-labeling.md](../specs/feature-r2-storage-encoding-labeling.md).

**프로세스 분리 (cloud-migration 완료 2026-05-08)**
- **프로세스 #1 = `backend.main:app` (uvicorn)** — HTTP API only. **현재: fly.io `petcam-api` (nrt, shared-cpu-1x 256MB, always-on)**, 도메인 `api.tera-ai.uk` 가 fly.io edge 에 A/AAAA 직결. 캡처 워커 / VLM 워커 in-memory 상태는 모름.
- **프로세스 #2 = `backend.capture_main` (asyncio)** — RTSP 캡처 + 인코딩 + R2 업로드 + Supabase INSERT. RTSP 가 LAN 의존이라 카메라와 같은 네트워크 (지금: 맥북 / 추후: 자체 HW) 에 가동.
- **프로세스 #3 = `backend.vlm_worker_main` (asyncio)** — 미라벨 클립 폴링 → Gemini 2.5 Flash → behavior_logs INSERT. RTSP 무관 → 클라우드 머신에 배포. UNIQUE(clip_id, source) 가 동시 워커 race 방어. **현재: fly.io `petcam-vlm-worker` (nrt, shared-cpu-1x 256MB, always-on)** — 2026-05-07 배포.
...
- 결정 락인: [`specs/cloud-migration-roadmap.md`](../specs/cloud-migration-roadmap.md) §4-3 + [`specs/feature-capture-worker-extraction.md`](../specs/feature-capture-worker-extraction.md) + [`specs/feature-vlm-worker-cloud.md`](../specs/feature-vlm-worker-cloud.md) + [`specs/feature-vlm-worker-fly-deploy.md`](../specs/feature-vlm-worker-fly-deploy.md) + [`specs/feature-labeling-web-cloud.md`](../specs/feature-labeling-web-cloud.md) + [`specs/feature-api-server-fly-deploy.md`](../specs/feature-api-server-fly-deploy.md).

**역할 분담**
| 컴포넌트 | 책임 | 이 레포 관리? |
|----------|------|--------------|
| Tapo C200 / IP캠 | RTSP 영상 송출 | X (외부 하드웨어) |
| Cloudflare DNS | `api.tera-ai.uk` A/AAAA → fly.io edge (DNS only) | X (Cloudflare 대시보드) |
| **petcam-lab #1 (API, fly.io)** | HTTP API + JWT + R2 signed URL | **이 레포** |
| **petcam-lab #2 (capture_main, 로컬)** | 캡처·인코딩·R2 업로드·DB INSERT | **이 레포** |
| **petcam-lab #3 (vlm_worker_main, fly.io)** | 미라벨 클립 자동 라벨링 (Gemini) | **이 레포** |
| **라벨링 웹 (Vercel)** | owner 검수 + 라벨러 큐 | **이 레포 `web/`** |
...
```
 ...
 [11] Flutter 앱: GET /clips  (Authorization: Bearer <supabase JWT>)
        ...
        └─► routers/clips.list_clips (user_id 필터 + seek pagination)
                                        │
 [12] Flutter: GET /clips/{id}/file/url  (Authorization 헤더)
        └─► routers/clips.get_clip_file_url
        └─► R2 signed URL 발급 (boto3.generate_presigned_url, 1h TTL)
        └─► JSON 응답 {"url": "...?X-Amz-Signature=...", "ttl_sec": 3600, "type": "r2"}
                                        │
 [13] Flutter: VideoPlayerController.networkUrl(signedUrl)
        └─► R2 가 직접 mp4 byte 서빙 (헤더 인증 불필요, API 서버 안 거침)
        └─► Cloudflare edge 캐싱 활용
                                        │
 [14] Flutter video_player 재생
```

**한 사이클 = 1분 세그먼트.** 캡처 워커는 하루 = 1440 세그먼트를 만들어낸다 (카메라당).

**fallback — `GET /clips/{id}/file` (Range)**: R2 키가 NULL 인 옛 클립 (R2 업로드 이전) 만 백엔드가 직접 byte stream. `clip.r2_key` 있으면 자동 redirect. R2 마이그레이션 후 신규 클립은 전부 위 [12]→[13] 흐름.
...
| ~~Cloudflare Named Tunnel~~ (폐기 2026-05-08, fly.io 로 이전) | 공유기 포트포워딩 불필요 + HTTPS 자동. 맥북 의존 + 슬립 시 502 문제로 always-on 이 필요해져 폐기. | [stage-d5-deploy-tunnel](../specs/stage-d5-deploy-tunnel.md) |
...
| **API 서버 fly.io 이전 (`api.tera-ai.uk` → fly.io edge)** | 사용자 맥북 의존 0. Cloudflare Named Tunnel 폐기 → DNS A/AAAA 직결 + Let's Encrypt 자동. API 서버는 always-on, 캡처 워커만 LAN 의존으로 남김. 같은 코드베이스 + Dockerfile.api / fly.api.toml 로 분리. | [feature-api-server-fly-deploy](../specs/feature-api-server-fly-deploy.md) |
| **R2 signed URL 패턴 (Flutter → R2 직접 GET)** | 영상 byte 가 fly.io 256MB 머신을 거치지 않게 함. API 서버는 DB 권한 체크 + 1h 유효 signed URL 발급만. R2 키가 Flutter 에 절대 노출 안 됨 (URL 만, TTL 만료 후 무효). `<video src>` cross-origin Authorization 한계도 해결 (signed URL 자체가 토큰). 같은 패턴 라벨링 웹 owner 흐름·Flutter 영상 재생 양쪽에 동일. | [feature-r2-storage-encoding-labeling](../specs/feature-r2-storage-encoding-labeling.md) + [feature-labeling-web-cloud](../specs/feature-labeling-web-cloud.md) |
