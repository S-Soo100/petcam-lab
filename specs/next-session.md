...
> **최종 갱신:** 2026-07-12 — **RBA 데이터 우선순위 확정.** 현재 병목은 router threshold가 아니라 다양한 운영 영상과 사람 GT다. 다음 착수점은 **라벨링 웹 v2 설계 → 카메라·개체·사육장 확대 → 클래스별 다양한 영상과 paired negative 수집 → 사람 blind GT**다. production VLM baseline·router-cost-v2는 이 데이터 생산 체계가 선 뒤에 동결·검증한다. Gate v3는 행동 분류/자동 skip이 아니라 bbox·best frame·trajectory evidence sensor로 shadow 운영한다.
