...
> **🟢 2026-08-01 사건 이어짐 Owner 자격검사 v2 — `DEPLOYED_VERIFIED_READY_FOR_OWNER_ELIGIBILITY`:** 기존 `0/60` 독립 pair 시험은 게코 가시성 자격검사 누락으로 사람 제출·해결 `0/0`을 확인한 뒤 `invalid_eligibility`로 보존했고, 삭제하거나 결과로 채택하지 않는다. 같은 historical media eligibility 정본에서 닫힌 6 camera-night·2 cameras의 연속 run을 골라 내부 인접 경계를 빠짐없이 포함한 development 120쌍(고유 clip 126)을 준비했다. R2 HEAD는 seed 직전 두 번 모두 `126/126`, payload digest `750b8a50…`이고 새 cohort는 `eligibility_open`, pair `120`, eligibility `0`, assignment `0`이다. Owner `bss.rol20@gmail.com` workspace는 `eligibility 0/120`+next pair+media 접근 1행, peer `dlqudan12@gmail.com`은 `waiting`+next pair 없음+media 거부를 production DB에서 확인했다. Owner는 `유효 / A 게코 없음 / B 게코 없음 / 둘 다 없음 / 촬영·재생 오류` 중 하나로 120쌍을 먼저 선별하며, 한 clip의 무효 판정은 그 clip을 공유하는 모든 경계를 제외한다. 유효 경계가 60개 이상일 때만 두 사람 사건 이어짐 배정을 원자 생성하고 holdout·행동 GT·기존 교차검수는 건드리지 않는다. implementation HEAD `bb798864a180aeff0008fbdeffb9d6d477976869`, production deployment `dpl_4ZNa8fMLtUktW17UxkMPqEEWfa6H` READY+`label.tera-ai.uk` alias를 확인했고 Owner Chrome에서 새 `0/120`·5개 선택지·A/B video `readyState=4/4`를 smoke했다. 사람 판정은 `0`으로 유지했다. [설계](../docs/superpowers/specs/2026-08-01-rba-sequence-eligibility-review-design.md) · [구현 계획](../docs/superpowers/plans/2026-08-01-rba-sequence-eligibility-review.md).
