...
> **🟡 2026-09-12 owner 방향: 2.6.1 뒤 '게코 미관측' 하이라이트 제외 → 장기 미저장.** 결정 로그 2026-09-12 제안 등록: A단계(큐·목록 기본 숨김)는 조건부 통과 — `gme-negative-audit-calibration-v1`(PENDING, 검출기 핀 v2.5→2.6.1 갱신 필요) 감사 상한 통과 뒤에만. B단계(미저장)는 CLAUDE.md '원본 삭제 금지' override·온디바이스 검출 전제라 보류. 표본 실측: 미관측 22/171 중 사람 O 1(4.5% 하한, Wilson 0.8~22%).
...
> 파일 선택에 더해 단일 파일 drag-and-drop을 지원하고, nested drag depth를 유지하며 비파일 drag는 기존
> 선택을 건드리지 않는다. 현재 provider는
...
> empty-state·권한 셸을 smoke했다. 후속 drag-and-drop은 Web `119 files / 1007 tests`, TypeScript,
> Python `1202 passed / 5 skipped`, Vercel production `dpl_CK5sogzYxLNpHJsaPmrH6fkqB1TC` READY 뒤
> 공개 화면 200·drop zone visible·console error 0·production fake 추론 503을 재확인했다.
> **R2 write·실제 worker/checkpoint·task/model/dataset seed는 0**이다.
...
> 측정했다. MiniCPM은 74/74 구조화 응답·same recall 57/57이지만 different 17/17도 모두 합쳐
> `REJECT_SAFETY`다. Qwen은 47/74까지 content가 전부 비어 schema 0/47이었고 swap delta가
> 1.0306GiB를 넘어 사전 규칙대로 중단해 `REJECT_RESOURCE/RELIABILITY`다. 두 모델 모두
> production/all-event shadow/자동 병합에 연결하지 않았고, holdout 접근·DB/R2/GT/service write는
> 0이다. 다음은 별도 TEST-SHEET에서 합성 입력만으로 Qwen structured-output 호환성을 분리한 뒤,
> 필요하면 MLX-VLM/llama.cpp 저메모리 경로를 비교하는 것이다. [보고서](../experiments/local-vlm-event-boundary-v1/REPORT.md) ·
> [TEST-SHEET](../experiments/local-vlm-event-boundary-v1/TEST-SHEET.md).
> **🟢 2026-08-02 사람 사건 경계 development 분석 v1 — `DEVELOPMENT_EVENT_GT_READY_FOR_LOCAL_VLM_BASELINE` / `EVENT_GT_READY_ROUTER_UTILITY_HOLD`:** 두 reviewer가 끝내고 Owner가 26개 불일치를 모두 해결한 sequence cohort를 Mac mini exact HEAD `319a341b14e5711569be35fc7c2215c2dd37b007`에서 SELECT-only로 분석했다. pinned count `120/74/148/148/26`, 최종 uncertain `0`, 입력 순서 변경 3회 hash 동일을 확인했다. 최초 판단 일치율 `64.86%`, Cohen's kappa `0.2645`, Owner 개입 `35.14%`, Owner 최초판단 채택 `65.38%`였다. 최종 경계는 `same_event 57 / different_event 17`로, 유효한 고유 clip `78`개를 사람 사건 `21`개로 묶어 `73.08%` 감소했다. 이 사람 사건 GT는 다음 local VLM baseline 입력으로 사용할 준비가 됐다. 반면 gap-only 후보는 `0/5/15초`에서 over-merge 0이지만 모두 57개 same 경계를 전혀 묶지 못하는 no-op이고, `30초`부터 over-merge `2`가 발생해 metadata-only 시간 router는 보류한다. 첫 one-shot은 DB float 왕복 최대 `3.979039320256561e-13`초를 exact 비교해 fail-closed됐고, `abs_tol=1e-9, rel_tol=0`과 `1e-6` 차단 테스트를 추가한 뒤 최초 `0600` salt를 재사용해 성공했다. private `0700/0600`, DB/R2/model/service mutation `0`, historical holdout·local VLM 실행 `0`이다. 다음 행동은 **별도 TEST-SHEET를 먼저 동결한 local VLM 사건 baseline**이며 production 채택·자동 skip은 금지다. [보고서](../experiments/rba-boundary-development-v1/REPORT.md) · [TEST-SHEET](../experiments/rba-boundary-development-v1/TEST-SHEET.md) · [설계](../docs/superpowers/specs/2026-08-02-rba-boundary-development-analysis-v1-design.md).
> **🟢 2026-08-02 경계 검수 블라인드 차단 + 공통 영상 다운로드 — `DEPLOYED_VERIFIED_WAITING_FOR_PEER`:** 두 검수자가 현재 cohort의 배정을 모두 끝내기 전에는 Owner에게 상대 답·불일치 수·해결 목록을 전혀 주지 않는 DB 원자 gate를 추가했다. 미완료 상태는 항상 `ready=false`, `items=[]`, `total=0`이고 해결 RPC도 같은 gate를 다시 검사한다. production migration `2026-08-02_rba_boundary_adjudication_blind_gate.sql`은 `terraaidev` Chrome의 정확한 Supabase 프로젝트에서 적용했고, service-role 전용 권한과 before/after-ready invariant를 확인했다. 현재 최신 경계 scope는 **peer completion pending**이다. Owner Chrome production smoke에서 `/labeling/boundary`는 `74/74`·상대 대기·해결 링크 미노출, 직접 `/labeling/boundary/conflicts`도 상대 답/진행 수 없이 대기 안내만 표시했다. 모든 라벨링 영상의 공통 `ReviewVideo`에는 기존 권한·scope 검사를 통과한 signed URL 다운로드를 추가했다. 구현 HEAD `2fbabfae695ef82023cc082e6eac1771d33293b8`, Vercel production `dpl_D9wrjG9FF5iPsF5Ay6xCdnEYsLSQ` READY+`label.tera-ai.uk` alias, 검증 Web `104 files / 943 tests`, TypeScript PASS, Python `1,055 passed, 5 skipped`, runtime PG 경합 probe 단독/병렬 PASS다. 다음 사람 행동은 **peer가 자신의 이어짐 확인을 모두 끝내는 것**뿐이며, 그전 Owner 해결·답 열람은 금지다. formal Blind30 v2는 별도 연구로, 현재 `NOT_READY` snapshot을 오늘 07:10 KST에 기존 Mac mini 작업에서 read-only 재검사하도록 1회 예약했다. [구현 계획](../docs/superpowers/plans/2026-08-02-rba-boundary-blind-hardening-download.md).
> **🟢 2026-08-01 사건 이어짐 자격검사 사유 분리 — `DEPLOYED_VERIFIED`:** Owner 자격검사 화면을 `유효 / 게코가 안 보임 / 실제 게코 활동 없음 / 영상 자체를 확인할 수 없음` 네 영역으로 나누고, 각 무효 사유는 A/B/둘 다를 따로 기록하도록 production DB와 웹을 확장했다. 22번은 이미 원본 `eligible`로 제출돼 있었으므로 원본 row·digest를 고치거나 지우지 않고 append-only 정정 1건을 추가해 effective decision만 `both_no_gecko_activity`로 바꿨다. production은 `eligibility_open`, 진행 `22/120`, 다음 ordinal `23`, correction `1`, 원본 22번 `eligible` 불변이다. Vercel `dpl_2Bid9YwwhE41HcfzyZ8j78ExQL7t` READY+`label.tera-ai.uk` alias이며 Owner Chrome에서 23번·네 영역·9개 선택지·A/B 자동재생을 확인하고 사람 답은 제출하지 않았다. 구현 HEAD `5ba33797b1158749af0c93754ef86353790817f4`, 검증 Web `103 files / 932 tests`, TypeScript PASS, DB 관련 `10 passed`, production Next build PASS, 독립 코드리뷰 Critical/Important/Minor `0/0/0`. 다음 사람 행동은 **23번부터 계속 자격검사**하는 것이다. [사유 분리 설계](../docs/superpowers/specs/2026-08-01-rba-boundary-eligibility-reason-groups-design.md) · [구현 계획](../docs/superpowers/plans/2026-08-01-rba-boundary-eligibility-reason-groups.md).
> **🟢 2026-08-01 라벨링 공통 영상 플레이어 — `DEPLOYED_VERIFIED`:** 라벨링 실제 mp4의 raw `<video>` 소유를 `ReviewVideo` 하나로 통합하고 모든 영상 경로에 음소거 자동재생·inline·preload auto를 적용했다. native controls는 제거하고 재생/시간/seek/소리/전체화면 조작바를 영상 프레임 아래에 분리해 촬영 타임스탬프를 덮지 않는다. 역할 셸은 navigation 220px+본문 1200px(`max 1484px`), 영상 상세는 정확히 `max 1200px`이며 320px에서는 조작바가 줄바꿈되고 수평 overflow가 없다. source가 바뀌면 player state를 key로 remount한다. 라벨링 production 소스의 공통 컴포넌트 외 raw video `0`, DB/API/R2/GT/사람 제출 변경 `0`. implementation HEAD `60935b513925294394c95bcbcc09b47868885b36`, Vercel `dpl_2x9PWrJycrsF6MiekwPDAAJ6qgsS` READY+`label.tera-ai.uk` alias. Owner Chrome smoke에서 A/B 모두 `readyState=4`, `muted=true`, `paused=false`, `controls=false`, currentTime `4.43→5.86`, control top=video bottom, desktop video width 약 `566px`, 320px scrollWidth=viewport를 확인했고 판정 버튼은 누르지 않았다. 검증: Web `103 files / 922 tests`, TypeScript PASS, production build PASS, Claude read-only review Critical `0`·Important `2` 보강 완료. [설계](../docs/superpowers/specs/2026-08-01-labeling-review-video-player-design.md) · [구현 계획](../docs/superpowers/plans/2026-08-01-labeling-review-video-player.md).
> **🟢 2026-08-01 사건 이어짐 Owner 자격검사 v2 — `DEPLOYED_VERIFIED_READY_FOR_OWNER_ELIGIBILITY`:** 기존 `0/60` 독립 pair 시험은 게코 가시성 자격검사 누락으로 사람 제출·해결 `0/0`을 확인한 뒤 `invalid_eligibility`로 보존했고, 삭제하거나 결과로 채택하지 않는다. 같은 historical media eligibility 정본에서 닫힌 6 camera-night·2 cameras의 연속 run을 골라 내부 인접 경계를 빠짐없이 포함한 development 120쌍(고유 clip 126)을 준비했다. R2 HEAD는 seed 직전 두 번 모두 `126/126`, payload digest `750b8a50…`이고 새 cohort는 seed 직후 `eligibility_open`, pair `120`, eligibility `0`, assignment `0`이었다. Owner `bss.rol20@gmail.com` workspace는 seed 직후 `eligibility 0/120`+next pair+media 접근 1행, peer `dlqudan12@gmail.com`은 `waiting`+next pair 없음+media 거부를 production DB에서 확인했다. Owner는 `유효 / A 게코 없음 / B 게코 없음 / 둘 다 없음 / 촬영·재생 오류` 중 하나로 120쌍을 먼저 선별하며, 한 clip의 무효 판정은 그 clip을 공유하는 모든 경계를 제외한다. 유효 경계가 60개 이상일 때만 두 사람 사건 이어짐 배정을 원자 생성하고 holdout·행동 GT·기존 교차검수는 건드리지 않는다. implementation HEAD `bb798864a180aeff0008fbdeffb9d6d477976869`, production deployment `dpl_4ZNa8fMLtUktW17UxkMPqEEWfa6H` READY+`label.tera-ai.uk` alias를 확인했고 Owner Chrome에서 초기 `0/120`·5개 선택지·A/B video `readyState=4/4`를 smoke했다. 이 수치는 배포 직후 snapshot이며 이후 진행량은 live workspace를 따른다. [설계](../docs/superpowers/specs/2026-08-01-rba-sequence-eligibility-review-design.md) · [구현 계획](../docs/superpowers/plans/2026-08-01-rba-sequence-eligibility-review.md).
...
> **🟢 2026-07-30 P4 Cam 2(dev) 7월 KST 40초 미만 media 정리 재분류 — 승인된 수동 정리:** 별도 task에서 owner가 직접 승인해 mp4 `1,468`개와 thumbnail `1,468`개, 합계 R2 object `2,936`개(약 `16.25 GiB`)를 삭제했다. 사람/legacy GT가 있던 clip `19`개와 object `38`개는 보존했고 대상 `2,936`개 부재를 검증했다. DB metadata는 유지하며 exclusion은 `media_deleted`, `worker_host=codex-manual-cleanup`이다. 사고·복구 대상이 아니며 추가 삭제도 하지 않는다. 자동 retention의 `SHORT_CLIP_RETENTION_DELETE_ENABLED=0`은 계속 운영 정본이다.
> **🟡 2026-07-31 Data Engine formal Blind30 v1 — `INVALID_SAMPLE_AFTER_FREEZE`:** frozen 30개를 실제 R2 `HeadObject`로 전수 대조해 exact object 25, missing 5를 확인했다. DB `r2_key` null과 missing 대상 system exclusion은 각각 0이어서 기존 metadata-only eligibility가 DB-R2 drift를 놓쳤다. human submission 0을 재확인한 뒤 공식 close RPC를 정확히 1회 호출했고 cohort closed, slots 60, awaiting consensus 30, submission 0과 기존 manifest를 보존했다. clip 교체·삭제·복구는 0이다. v2는 [`TEST-SHEET`](../experiments/rba-data-engine-blind30-v2/TEST-SHEET.md)와 [설계](../docs/superpowers/specs/2026-07-31-rba-data-engine-formal-blind30-v2-design.md)에 따라 v1 T0 이후 future pool, 실제 R2 HEAD 30/30 이중 preflight, 새 forward-only RPC로 다시 동결한다. threshold·selection 균형·`motion-blind-v1` comparator는 불변이다. backlog 300 human-first Gate 감사는 별도 pending이다.
> **🟢 2026-07-31 B그룹 07-29 slot 0 — `GROUP_B_SLOT_ROOT_CAUSE_CONFIRMED_NO_BUG`:** active member 2명·camera 3대였지만 해당 activity-day의 raw clip/eligible/ownership/slot이 전부 0이었다. Cam 2·3은 window 시작 전에 마지막 clip이 끝났고 Cam 4는 전체 clip 0이라 materializer 입력 자체가 없었다. 기존 slot·assignment 수정은 하지 않았다.
> **🟢 2026-07-30 owner tutorial smoke — `OWNER_TUTORIAL_SMOKE_COMPLETED`, owner first-5 superseded:** fingerprint `05fd4fe03dc3` owner 계정이 production `tutorial-v1` run 1의 position 1~5를 모두 완료했다(22:53:10~22:56:36 KST, waiver 0). 최신 화면·교육·저장·완료 전환 UX smoke로만 유효하다. owner는 tutorial gate를 우회하고 이미 daily double-blind·불일치 검수를 수행하므로 owner first-5와 `FORMAL_NON_OWNER_PILOT_PENDING` 지시는 중단·superseded다.
> **🟢 2026-07-25 짧은 오류 영상 visibility-first 운영 수용 — `SHORT_CLIP_VISIBILITY_FIRST_DEPLOYED_ACCEPTED` (당시 snapshot):** production migration·Fly/Vercel·app/web RLS·Mac mini `ENABLED=1/WRITE=1/DELETE=0` 적용과 RunAtLoad kickstart를 검증했다. **07-25 당시** 원장 exclusions **823**(candidate 783 / quarantined 40), off-policy 격리 0, quarantined 40건은 기존 Owner `skip` 40건과 일치하며 `label/hold` 0, media_deleted 0, active delete lease 0이었다. Owner 결정으로 추가 예약 감시와 자연 hourly cycle 1회 관측은 면제해 **미관측 사실을 보존한 채 운영 수용**으로 마감했다. 자동 retention의 R2 물리 삭제는 계속 비활성이다. `media_deleted=0` 현재값 해석은 위 07-30 승인 수동 정리 기록으로 superseded됐고, 아래 `READY_FOR_DEPLOY` 블록은 구현·배포 전 이력으로 superseded.
> **🟡 2026-07-25 짧은 오류 영상 visibility-first — `SHORT_CLIP_VISIBILITY_FIRST_READY_FOR_DEPLOY`(격리 worktree `codex/short-clip-visibility`, handoff base `d9909906`):** 물리 삭제를 운영 목표에서 제외하고 app/web 노출 차단을 우선하는 owner 승인 작업. TDD RED→GREEN Task 1~5, 커밋 4개 + docs. **① forward migration `2026-07-25_short_clip_visibility_first.sql`** — `fn_restore_short_clip_exclusion` 재정의(시스템 격리만 restored, `motion_clip_labeling_triage`/`_events`/`owner_decision` 미터치 = 07-25 canary 사고 해소) + helper `fn_motion_clip_visible_to_owner`(SECURITY DEFINER STABLE search_path='') + `ALTER POLICY "own clips select"`(security_invoker view 상속). **② 실 PG15 rollback probe 3회**(`RESTORE_TRIAGE_IMMUTABLE_OK`/`APP_RLS_VISIBILITY_OK`/`PROBE_RESIDUE=0`, skip/label triage md5 불변·active lease PT409·RLS 가시성). **③ backend media guard** `ensure_clip_media_visible`(quarantined→404·media_deleted→410·DB실패→502·signer 0회, 4개 엔드포인트). **④ 웹 문구** `자동 제외만 해제`+사람판정 유지 안내+성공 notice(payload/endpoint 불변). 검증 pytest **828**·web **841**·tsc 0·git diff --check clean. 금지 감사: R2 delete/DELETE_ENABLED=1/triage write/Flutter 변경/raw 응답 = 0. **⚠️ Task 6 미실행: main FF merge·production migration apply·Vercel/Fly deploy·app/web smoke·Mac mini `1/1/0` 재설치·자연 cycle = 전부 배포 단계 대기.** 보고 [`short-clip-visibility-first-report`](../docs/handoff-prompts/2026-07-25-short-clip-visibility-first-report.md).
> **🟢 2026-07-24 권한별 라벨링 웹 — `ROLE_BASED_LABELING_WEB_READY_FOR_DEPLOY_REVIEW`(격리 worktree `codex/role-based-labeling-web`):** 라벨링 웹을 로그인 역할 전용 홈+정확히 3메뉴로 재편(TDD RED→GREEN Task 1~9, 커밋 8개 + docs). 순수 역할/경로 계약(`labelingRoleNavigation`·`labelingRouteAccess` 8-카테고리, 역할 판정 Owner→라벨러→미승인) · forward-only read RPC 3종 `2026-07-24_role_based_labeling_reads.sql`(`fn_list_motion_blind_history`/`fn_list_motion_labeling_library`/`fn_get_motion_blind_owner_overview`, service_role 전용·write 0·정적 계약 3) · allowlist read API 5(history owner→403·library 양역할·library[clip]·media·owner-overview owner-only) · 반응형 `RoleShell`(320px 하단 3탭 / lg 220px 사이드, emerald 아웃라인) · 라벨러 오늘작업(활동일 07:00 KST·완료 후 이전 활동일)/내기록(본인 immutable·확정됨/검수 중) · 읽기전용 영상보관함(6필터·확정 전 GT 은닉·출처 구분·write 0) · Owner 운영홈/연구허브/역할인식 Canary union(라벨러=자기작업·Owner=집계현황) · 미승인 단일화면 · 정적 반응형 감사(`npm run audit:labeling-role-ui`). 검증 web **797**·tsc 0·pytest **757**·audit 통과·git diff --check clean. ⚠️ `npm run build`는 dangerous-guard(donts#9) 차단 → `BUILD_UNVERIFIED_SAFETY_HOOK`(tsc를 빌드 증거로 쓰지 않음). **migration apply·main merge·Vercel deploy·실제 group/Canary 변경은 0(Preview Deployment Gate = migration rollback probe → Vercel preview → 6-width(320/360/390/768/1024/1440) 스크린샷 → 역할 브라우저 canary, 별도 owner 승인).** 편차 2건(보고): library `final_decision`은 RPC 파라미터 부재로 page-scoped client 필터 · team "Canary 생성"은 전용 페이지 부재라 `/labeling/blind/groups` 임시 링크(후속). 보고서 [`role-based-labeling-web-report`](../docs/handoff-prompts/2026-07-24-role-based-labeling-web-report.md).
> **🟡 2026-07-24 이중 블라인드 preview 배포 + 12클립 canary 준비 — `DOUBLE_BLIND_LABELING_CANARY_READY_FOR_HUMAN_REVIEW`(격리 worktree `codex/double-blind-labeling-hardening`, HEAD `2dfcaeda`):** owner 승인 외부 변경 범위에서 Gate A~E 수행. plan `docs/superpowers/plans/2026-07-24-double-blind-labeling-preview-canary.md` + gitignored manifest `storage/handoffs/2026-07-24-double-blind-labeling-preview-canary-handoff.md`(HANDOFF_OK runtime=oneshot@slxjvzz). **① production Supabase(`slxjvzzfisxqwnghvrit`, MCP 확인)에 forward migration `motion_double_blind_labeling` 적용**(atomic, apply_migration). 검증: 9테이블+RLS·client policy 0·anon/auth 권한 0·service_role 전용·append-only trigger 6·**14 함수 body 정규화 md5 가 tracked 파일과 byte-identical**·advisor 신규 critical/error/WARN 0·**기존 13 사람테이블 지문 byte-identical(mutation 0)**·rollback residue 0. **② Vercel preview READY**(`dpl_AGBsRynz…`, `petcam-py7zs6b20-…vercel.app`, Deployment Protection 있음). ③ **12클립 canary 준비**: 전용 그룹 `blind-canary-preview-20260724`(active member 2, **active camera 0** — P4 Cam 4(0clip) 부여 후 종료로 live materialization 차단), cohort_kind='canary' cohort, `P4 Cam (dev)` fresh·재생가능·시간분산 clip 12 × reviewer 2 = **canary slot 24, consensus 12 awaiting, submission 0, live slot 0**. reviewer=`크랑이아빠`(d5e66448, owner 2nd)·email-resolved labeler(dad1a07e), 각 정확히 1 resolve. ④ blind: queue RPC 12행·peer/제출상태 미노출(구조적). **⚠️ 미검증: preview env→production Supabase 연결(Vercel 보호로 미확인, owner 진입 시 canary 표시로 확인)·`npm run build`(hook 차단)·사람 브라우저 워크스루(계정 로그인 불가).** **금지 준수: main merge·prod deploy·prod env 변경·live mapping·기존 GT/label 수정·151 재사용/manifest·사람 대신 제출·force push = 전부 0.** 사람 12개 blind 제출 전이라 DEPLOYED/PRODUCTION_READY 주장 금지. 카메라 해석("P4 Cam"→`P4 Cam (dev)`)·canary 제거(close RPC)는 보고서 참조. 보고서 [`canary-ready-report`](../docs/handoff-prompts/2026-07-24-double-blind-labeling-canary-ready-report.md).
> **🟢 2026-07-24 그룹 이중 블라인드 라벨링 하드닝 — `DOUBLE_BLIND_LABELING_HARDENED_READY_FOR_DB_PREVIEW`(격리 worktree `codex/double-blind-labeling-hardening`):** 아직 production 미적용인 `2026-07-23_motion_double_blind_labeling.sql` 을 같은 feature 계열에서 하드닝(새 커밋만 추가, 기존 push 커밋 amend/rebase 없음). ①`fn_ensure` aggregate `FOR UPDATE` 런타임 오류 제거(멤버 잠금·집계 분리) + live clip ownership 을 consensus anchor 로 고정(slot 0/2·PT425, 세 번째 slot 방지) `4afb091` ②submit 이 공유 consensus row 를 slot 보다 먼저 `FOR UPDATE`(둘째 제출이 첫째 관측) + finalize 교차 객체 identity fail-closed(22023) + `auto_compared` transition-only event `846021f` ③GT 를 `GT_DURATION_CAP=3600` 대신 실제 clip duration 으로 검증(`getAssignedBlindClip`/`getOwnerClipDuration`, 미배정=404 은닉) `69eebb0` ④`motionBlindDraft.ts` 브라우저 임시본(user·clip·cohort·version 격리, lease/상대/VLM/evidence/r2_key 미저장, duration-aware) + detail 배선 `8f3e9f3` ⑤canary 자격 3중 EXISTS(labelers+승인+group active member) + `isValidActivityDay` 실제 달력 검증 `c508970` ⑥disposable postgres 동시성 실증 러너(docker+local backend) + prerequisites/rollback probe SQL + 단위 테스트 `c2daf4b`. **✅ owner 로컬 Homebrew PostgreSQL 15 임시 DB(`blind_probe_*`)에서 세 마커 실측(3회 안정): `DB_RUNTIME_PROBE_OK`(hardening_probe.sql 14 assertion 전량)·`DB_CONCURRENCY_PROBE_OK`(두 세션 실제 동시 제출, consensus 잠금 직렬화, peer_present {f,t})·`PROBE_RESIDUE=0`.** 안전 계약 전량 준수(prefix 검증 임시 DB만 create/drop·127.0.0.1 only·기존 DB/role 무손상·docker backend 보존·probe 생성 role self-clean). ⑦**Codex 독립 리뷰 P1 3건 TDD 반영**: finalize 가 각 제출의 slot identity(clip/group/reviewer/cohort)를 잠금·재조회 검증(22023, forged slot 실 DB assertion) · ensure 는 reviewer 자기 membership 선잠금 제거→공유 멤버셋부터 동일 순서 잠금(deadlock 해소, 두 reviewer 동시 ensure 실 DB probe로 2 slot 증명) · local runner 는 기존 role 조회 실패를 fail-closed(빈 집합 금지)+dropdb/role cleanup returncode 검사(기존 role 미삭제 단위 테스트). 검증 pytest **754**·web **723**·tsc 0·diff clean, 세 마커 재확보(4회 안정·무흔적). `npm run build` 만 dangerous-guard(donts#9) 차단(owner-unverified). **상태: migration production applied=false · main merged=false · Vercel deploy=false · 실제 group mapping=false · canary 생성=false · Owner Pilot 151 manifest=미시작.** 다음 게이트 = Codex 하드닝 diff+DB evidence 리뷰 → 별도 preview deployment handoff(safe DB apply + owner 승인 12-clip 격리 canary) → 수용 후 main/production. 보고서 [`double-blind-labeling-hardening-report`](../docs/handoff-prompts/2026-07-24-double-blind-labeling-hardening-report.md).
> **🟢 2026-07-23 그룹 이중 블라인드 라벨링 — `DOUBLE_BLIND_LABELING_READY_FOR_DEPLOY_REVIEW`(격리 worktree `codex/double-blind-labeling-groups`):** 승인 라벨러 2인 그룹이 담당 카메라 같은 `motion_clips` 영상을 상대 답 못 본 채 각각 한 번 판정→결정론적 일치 자동합의·불일치만 owner 검수하는 운영 계층을 forward-only로 독립 추가(TDD RED→GREEN, Task 1~7 커밋 분리). ①순수 comparator/활동일 계약(`motionBlindReview.ts`, `motion-blind-v1`·segment≤500ms·배열 dedup·note 제외) ②forward migration `2026-07-23_motion_double_blind_labeling.sql`(9테이블 group/member/camera/cohort/progress/slot/submission/consensus/events + service-role RPC 11개, `SECURITY INVOKER search_path=''`·row lock·append-only·안정 SQLSTATE 11종, 정적 계약 테스트 37) ③labeler read API(workspace/queue/canary/detail/media, scope-embedded cursor·allowlist 매퍼·상대원문 0) ④submit/consensus/claim(immutable 제출+서버 비교+digest finalize 멱등, per-tab lease 응답 미노출) ⑤버튼 상태(`SelectionChip/Card`+labeling Button variant, 검은 active 제거)·쳇바퀴 단계형(ride→rotate→push, payload 불변) ⑥labeler 활동일 큐/온보딩/상세/canary + 홈 role 스위치 ⑦owner 불일치 검수/그룹배정/canary API+UI. 검증 web **705**·tsc 0·pytest **731**·git diff --check clean. ⚠️ `npm run build`는 dangerous-guard(donts#9) 차단 — tsc를 빌드 증거로 쓰지 않음, owner 터미널/Vercel preview 필요. **migration apply·main merge·deploy·실제 그룹 매핑·preview canary는 0(설계 §11 Task 8 = 별도 owner 승인 경계).** 보고서 [`double-blind-labeling-groups-report`](../docs/handoff-prompts/2026-07-23-double-blind-labeling-groups-report.md).
...
> **[⛔ SUPERSEDED 2026-07-30 production evidence — 아래는 07-14 snapshot] 라벨링 웹·튜토리얼 production 배포 완료, 당시 운영 pilot 대기.** 가입·승인·날짜 선택·GT/VLM 공통 폼·owner 보정 감사·튜토리얼 서버 게이트가 production에 반영됐다. owner가 `e679f8ad`를 drinking target `glass`, `d9346cbe`를 hand_feeding context `human`으로 보정해 revision 2건을 남겼고, position 1~5 의미 preflight 5/5 후 `tutorial-v1`을 seed·활성화했다. active set 1·lesson 5·distinct position/clip 5·reference/prediction/review/feedback 5/5, seed 전후 production GT row 수 불변을 확인했다. 2026-07-14 운영 재확인은 승인 대기 0명·활동 중 라벨러 2명·두 명 모두 `tutorial-v1` 0/5, console error 0이다. 최종 검증 web 245·tsc·Vercel production build(`dpl_5nsq7hqnKHcwKrZefqk7YozsrhmE`)·Python 334, 최종 코드 커밋 `0702e66`. 당시의 “한 명 tutorial 5/5→첫 본작업 5개” 지시는 위 2026-07-30 non-owner production evidence로 충족·superseded됐으며 반복하지 않는다. 공통 쉬운 말·입력 복구 코드는 배포됐고 `tutorial-v2` 콘텐츠 전환은 실제 이해 문제를 보고할 때만 재개하는 조건부 backlog다.
