...
> **활동필터 v0 — shadow 운영 중 (정본, 2026-07-14):** 파이프라인 + migration 3종(v0/evidence_identity/identity_frames=**7컬럼**, FK=motion_clips, fail-open). **P4 Cam dev 3대는 제품 owner 계정과 분리된 테스트 계정 소유**(전체 UUID 미기재).
...
> **현재 상태(정본):** **A** policy-version guard 완료 · **B** 세 레포 main fast-forward 통합 완료(gate `89237a5`·nightly `e4e7cc6`·lab main) · **C** shadow launchd 상시 가동 완료(`com.petcam.activity-worker`·StartInterval 3600·RunAtLoad·env `ACTIVITY_POLICY_VERSION=activity-v1`) · **D evidence 축적 진행 중**. settings **3대 enabled=true · exclude_absent_enabled=false · exclude_static_enabled=false · active_policy_version=activity-v1**. assessments **205 부터 자동 증가**(전부 policy v1·threshold 0.10·identity 7컬럼 완전). 앱 **effective_activity_sec = raw_duration_sec · 실제 exclusion 0**.
> **다음 checkpoint:** 최소 **3개 날짜 축적 + 30분 episode dedup + 독립 static ≥20** 시 사람 blind 검수(REPORT §13, E~G 미실행). **exclude_absent REJECT · exclude_static HOLD 유지.** Flutter 연결·실제 시간 차감·exclude 스위치 활성화·Gate/DB schema/feature 삭제 금지. 상세 REPORT §8~§15 + 메모리 `activity-filter-v0-preflight-reject`.
