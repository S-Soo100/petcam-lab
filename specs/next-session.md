...
> **🔴 최종 갱신 2026-07-21 — ChatGPT 연구 정본 통합선 확정:** 연구 최종 책임은 ChatGPT/Codex가 맡고 Claude는 승인된 구현·실험 수행자로 전환했다. 활성 정본은 [`docs/research/ACTIVE-RESEARCH.md`](../docs/research/ACTIVE-RESEARCH.md). `petcam-lab`과 `petcam-nightly-reporter`의 논리 통합 브랜치는 `codex/research-consolidation-20260721`이며, nightly는 최신 Python Evidence main과 Claude P1/결정론 연구 이력을 merge하되 **REJECT된 basking v4.1 runtime은 활성 코드에서 분리**했다. 다음 순서는 W1(07-22 아침 SELECT-only 밤 사이클 검증) → W2(T2 GT 엔진 spec only) → W3(T1 v2는 decision-gate 재통과 시 선택)다. Python Evidence 실패 Slack 링크는 연구 통합 뒤 별도 운영 하드닝으로 재개한다.
