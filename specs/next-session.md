...
> **활동필터 v0 (2026-07-14):** Claude 없이 Mac mini RF-DETR로 비활동 clip 을 앱 활동시간서 제외하는 파이프라인 **완성 + migration 운영적용**(gate `feat/activity-filter-gate` ffbe6dc · nightly `feat/activity-worker` faff550 · lab `feat/activity-filter-migration` 69cecc8/1064278/next, 미push). `clip_prelabels`/`clip_activity_assessments`/`camera_activity_filter_settings`+`v_clip_effective_activity` view(FK=motion_clips, fail-open). **그러나 카메라A 사람 preflight = 두 스위치 reject**: exclude_absent 10/10 false(RF-DETR 게코미검출 FN·precision0%)+exclude_static 1건. 활성화 보류, 앱 활동시간 raw 유지. **재개 전제=detector recall(Gate v3 재학습, 이번 FN 10건=hardcase)**. launchd 미설치·Flutter(Phase4) 미착수. 상세 [`experiments/activity-preflight-0714/REPORT.md`](../experiments/activity-preflight-0714/REPORT.md) + 메모리 `activity-filter-v0-preflight-reject`.
