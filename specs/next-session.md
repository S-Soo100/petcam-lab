...
> **🟡 2026-07-31 RBA 사건 단위 전수 분석 — `PHASE1_PLAN_READY_HANDOFF_PENDING`:** owner는 ① 에러·흔들림이 아닌 실제 게코 활동 원본은 전부 열람 가능 ② 모든 활동 사건은 최소 하나의 AI 분석 결과 보유를 제품 계약으로 승인했다. 원본 파일은 합치거나 대체하지 않고 같은 카메라의 연속 clip을 논리적 사건으로 묶는다. Python/OpenCV Evidence와 Gate는 사건 경계·대표 프레임을 돕는 sensor이며 자동 skip 판정기가 아니다. 목표 cascade는 `모든 사건 local VLM 1차 → 모호·희소·케어 중요 cloud VLM/SegmentVLM → 충돌·고가치 사람 검수`다. 구체 행동 근거가 없으면 일반 활동·게코 안 보임·artifact·판정 불가로 정직하게 완료한다. 현재 Data Engine, formal Blind30 v2, backlog 300 human-first Gate 감사 우선순위는 유지한다. **Phase 1 사건 묶기 shadow**의 owner 승인, Fable 5 교차리뷰, TEST-SHEET와 구현 계획까지 끝났고 Mac mini 전용 worktree handoff만 남았다. DB·R2·서비스·모델·앱 변경은 아직 0이다. [설계](../docs/superpowers/specs/2026-07-31-rba-event-first-total-coverage-design.md) · [TEST-SHEET](../experiments/rba-event-grouping-shadow-v1/TEST-SHEET.md) · [구현 계획](../docs/superpowers/plans/2026-07-31-rba-event-grouping-shadow-v1.md).
