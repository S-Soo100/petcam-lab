...
> **최종 갱신: 2026-07-15 — 야간 Claude VLM 후보 shadow 재가동.** 아래 VLM 블록이 현재 실행 정본이며 라벨링 웹·튜토리얼 상태는 그대로 유지된다.
> **라벨링 후보 격리함 — DB 적용·Web 배포 전(2026-07-15):** 라벨링 가치가 낮아 보이는 `camera_clips`를 owner 전용 격리함으로 라우팅하는 작업 큐(삭제·자동GT 아님). 코드·H7 하드닝 완료, DB `clip_labeling_triage`+append-only events+service_role RPC를 production에 적용했다. Supabase 기본 함수 권한으로 남은 가드 트리거 직접 실행권한은 후속 `2026-07-15_labeling_triage_guard_execute_revoke.sql`로 anon/authenticated/service_role 모두 회수했고 advisor 신규 경고 0이다. 실제 rollback probe에서 quarantine·skip 세션 차단, owner label·system label·triage 없음 허용, stale 상태, 중복 이벤트 no-op, 감사 이벤트 UPDATE/DELETE/TRUNCATE `0A000`을 확인했으며 `triage=0/events=0/sessions=33`으로 원복됐다. owner API·3탭 격리함·촬영일/카메라 필터·영상 검토·auto-next·본 큐 수동격리 버튼을 구현했고, H7 request generation으로 목록/상세/영상의 늦은 응답이 새 화면을 덮는 문제를 차단했다. **검증 web 340 tests·tsc, Python 336, whitespace clean. 다음:** Web commit/push·Vercel 배포→owner/labeler E2E→`petcam-nightly-reporter` worker read-only preview 30→owner 오격리율→canary→제한 backfill. worker 전까지 격리 데이터 0이라 기존 라벨링 큐 동작은 유지된다. 설계 [`2026-07-15-labeling-triage-quarantine-design.md`](../docs/superpowers/specs/2026-07-15-labeling-triage-quarantine-design.md) · 계획 [`2026-07-15-labeling-triage-quarantine.md`](../docs/superpowers/plans/2026-07-15-labeling-triage-quarantine.md).
> **야간 VLM 후보 shadow(운영 중):** Mac mini의 `com.petcam.vlm-candidate-worker`가 22·00·02·04시 KST에 직전 2시간을 본다. 카메라·창당 최대 4개를 `하이라이트/미세행동/다양성/제외감사` 슬롯으로 고른 뒤, clip당 6프레임을 카메라별 **Claude Code 구독 1회 batch**로 분석한다. provider=`claude_cli_batch`, exact model=`claude-sonnet-5`; API key는 쓰지 않고 DB 실제/예약 비용은 0원, 환산 비용만 result provenance에 남긴다. 첫날 후보 8개는 사람 검토상 모두 유효했고 최종 8/8 분석 성공(`moving` 7·`unseen` 1), 요청/실제 모델도 모두 일치했다. 실제 API 청구액은 `$0`, 두 batch API 환산 참고비용은 합계 약 `$1.179497`다. 02~04시 4건의 최초 실패 원인은 영상·모델·인터넷이 아니라 **LaunchAgent의 `USER`·`LOGNAME` 누락으로 Claude CLI가 Keychain 구독 인증을 찾지 못한 것**이었다. launcher 환경과 worker 인증 preflight·안전한 오류코드를 보강했고, GUI launchd domain에서 Max 구독 로그인 확인 후 기존 실패 job만 attempt 2로 재시도해 4/4 성공·`error_code=NULL`을 확인했다. 결과는 `clip_vlm_selector_runs/jobs`에만 남으며 `behavior_logs`·`camera_clips`·앱 하이라이트·활동시간은 변경하지 않는다. 즉시 중단은 `launchctl bootout gui/$(id -u)/com.petcam.vlm-candidate-worker`. **다음 checkpoint:** 다음 22시 자동 실행이 인증 preflight를 통과하는지와 1박 후보 utility/구독 한도 효율을 계속 관찰한다. 검수 전 고객 노출·자동 라벨 승격은 금지한다. nightly 인증 복구 commit `7216b16`, lab migration commit `c37e307`.
...
> **현재 상태(정본):** Mac mini LaunchAgent는 테스트 카메라 3대를 매시간 `activity-v1`으로 분석한다. Flutter main `2e4fced`(`v0.20.1+35`, 기능 커밋 `b4db4dc`)은 홈·카메라 상세·어젯밤 리포트의 활동시간을 `v_clip_effective_activity.effective_activity_sec`로 읽고 view 오류 시 raw로 fail-open한다. **카메라 A 한 대만 `exclude_static_enabled=true`**, 다른 두 대는 false, **세 대 모두 `exclude_absent_enabled=false`**, policy는 `activity-v1`이다. iPhone 17 simulator에서 홈·상세·오늘/어제·리포트 문구/렌더를 확인했고, 카메라 A 어제 값이 `8h 50m → 8h 45m`로 변경됐다. DB 실측은 static 10 clip·320.4초 차감, static zero 위반 0·absent raw 유지 위반 0이다.
