...
> **🟡 2026-07-23 운영 라벨링 v3 연속 검수 UX — `BLOCKED_PREVIEW`(코드·회귀·CI build 완료, owner canary·배포 미완):** motion v3 상세에서 `보류/제외` 성공 후 카테고리 탭으로 강제 이동하던 동작을 없애고, owner `/labeling/motion` 기본 탭을 `미분류`로 바꾼 뒤 결과 확인·결정 취소(undo)·`다음 미분류 영상`(신규 owner 전용 `GET /api/labeling-v3/{clipId}/next`, 기존 큐 RPC keyset 재사용, migration 0)을 한 흐름으로 만들었다. Task 1~4 TDD RED→GREEN 커밋 `5830773`·`a6e9814`·`c078727`·`f43ccd8`, 브랜치 `codex/motion-labeling-continuous-review-ux` push(`f43ccd8`). **검증:** web **527** vitest·pytest **694**·`tsc --noEmit` 0·`git diff --check` 0·금지범위(migration/RPC/GT·session·event 삭제/legacy·tutorial/자동 next/open redirect) 전부 0. **실 Next build는 세션 안전 훅이 in-session 실행 차단** → **Vercel preview build `state=success`로 대체 검증**(deploy `F3jKvjHLj7zyyYBN7Ddvgd9AVUrG`). **미완(owner 인증·터미널 필요):** ① preview 10-canary(skip3/hold3/label2/경계2) ② `--ff-only` main 통합(현재 `origin/main` `4b48038` 불변, FF-safe) ③ production 배포 + reversible canary reset. **다음:** owner가 preview에서 10-canary 통과 확인 → `--ff-only` main → Vercel production → production canary(제외→결정취소, 보류→다음)·전부 `reset`. 보고서 [`continuous-review-ux-report`](../docs/handoff-prompts/2026-07-23-motion-labeling-continuous-review-ux-report.md).
