...
> **야간 VLM 후보 shadow(운영 중):** Mac mini의 `com.petcam.vlm-candidate-worker`가 22·00·02·04시 KST에 직전 2시간을 본다. 카메라·창당 최대 4개를 `하이라이트/미세행동/다양성/제외감사` 슬롯으로 고른 뒤, clip당 6프레임을 카메라별 **Claude Code 구독 1회 batch**로 분석한다. provider=`claude_cli_batch`, exact model=`claude-sonnet-5`; API key는 사용하지 않고 DB 실제/예약 비용은 0원, 환산 비용만 result provenance에 남긴다. 2026-07-15 02:27 canary는 44개 중 4개 선택→4/4 성공, 요청/실제 모델 일치, launchd 강제 smoke exit 0·중복호출 0을 확인했다. 첫 네 결과는 모두 `moving`, batch 1회의 API 환산 비용은 약 `$0.530825`였다(실제 구독 청구 0). 따라서 **가동 성공과 후보 utility/구독 한도 효율은 별개**이며 1박 검수가 필요하다. 결과는 `clip_vlm_selector_runs/jobs`에만 남으며 `behavior_logs`·`camera_clips`·앱 하이라이트·활동시간은 변경하지 않는다. 기존 activity worker도 runs 7·last exit 0으로 생존한다. 즉시 중단은 `launchctl bootout gui/$(id -u)/com.petcam.vlm-candidate-worker`. **다음 checkpoint:** 04시 자동 실행 결과와 1박 token/후보 슬롯/사람 유효성을 확인하되, 검수 전 고객 노출·자동 라벨 승격은 금지한다. nightly 기능 commit `841220c`, lab migration commit `c37e307`.
