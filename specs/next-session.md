...
> **🟡 2026-07-31 RBA 사건 단위 전수 분석 — `PHASE1_HARNESS_IMPLEMENTED_VERIFIED_PRODUCTION_PREPARE_AUTH_BLOCKED`:** owner는 ① 에러·흔들림이 아닌 실제 게코 활동 원본은 전부 열람 가능 ② 모든 활동 사건은 최소 하나의 AI 분석 결과 보유를 제품 계약으로 승인했다. metadata accounting/event core, exact 12 camera-night·120 boundary pair 동결, 독립 GT scorer, SELECT-only runner를 Mac mini 격리 worktree에서 구현했다. 독립 리뷰 후 denominator를 선택한 exact 12박 전수로 고정하고, dev/holdout GT 파일을 물리 분리했으며, holdout metrics는 freeze 이후 raw reviewer GT와 3-run grouping에서 직접 계산한다. 모든 live/canary/formal review slot 이력을 차단하고 7개 이상 camera 및 split-wide bin inventory의 false blocker도 제거했으며 pair/source manifest의 snapshot·blocked-set provenance도 결합했다. 최종 code HEAD `f1fcba03d9ad3f93926f372a4bc504b6237a5fc1`에서 Mac mini·local focused **54 passed**, local full suite **994 passed, 5 skipped**다. Mac mini predecessor full suite의 환경 의존 실패 3개(다른 사용자 절대경로 1, PostgreSQL role probe 2개)는 별도 기록했다. mutation/RPC·R2·모델·frame·service 호출은 모두 0이다. 현재 process에 production Supabase SELECT credential과 기존 CLI project link가 없어 실제 source snapshot, private manifest, threshold 0의 3회 실행은 시작하지 않았다. credential 추출로 우회하거나 cutoff/bin/camera-night를 완화하지 않는다. 따라서 `READY_FOR_HUMAN_BOUNDARY_GT`를 아직 주장하지 않으며 다음 동작은 동일 exact code HEAD에 인증된 SELECT-only 환경을 주입해 prepare를 한 번 실행하는 것이다. [설계](../docs/superpowers/specs/2026-07-31-rba-event-first-total-coverage-design.md) · [TEST-SHEET](../experiments/rba-event-grouping-shadow-v1/TEST-SHEET.md) · [구현 계획](../docs/superpowers/plans/2026-07-31-rba-event-grouping-shadow-v1.md) · [보고서](../experiments/rba-event-grouping-shadow-v1/REPORT-TEMPLATE.md).
