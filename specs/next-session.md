...
> **🟠 2026-07-22 Evidence B1R2 — verdict `recoverable coverage OPEN(드레인 중)`, NOT `B1R2_DATA_AVAILABLE`(B2·GT·Local VLM 정지):** B1R 의 "역사 media 보존창 전량 소실"이 **oldest-first canary 과일반화**임을 실측 정정. R2 inventory 직접 대조(prefix `terra-clips/clips/` 실측 — 초기 `clips/` 가정은 HEAD mismatch 159 로 fail-closed 후 정정): study_total 16786 = succeeded 2841 + **silent(복구가능) 13837** + terminal 0 + **source_expired 108** + open 0. **소실은 108건뿐**(B1R 13945 는 wrong-prefix 오류). availability_sha256 `e69baec9`, 독립 recompute MATCH, bounded HEAD available 173/173·expired 42/42 mismatch 0. **failure typing 배포**(404→source_media_missing terminal·403→r2_access_denied·5xx→retryable; nightly origin/main FF **`6ab6dd2`**→Mac mini pull, focused 56 PASS)+**forward migration apply**(11-code CHECK, probe 11 accepted/bogus rejected/0 leaked, job 분포·advisor 불변). **canary `B1R2_CANARY_VERIFIED`**(silent round-robin 30, HEAD 30/30, succeeded 30/run 30/실패 0/temp 0/exit 0/drift 0). **bulk** cap-500 enqueue 500 + 4 cycle 179 succeeded/실패 0/신규 media-missing 0. **recoverable closure 미완주**(open 381 + silent 13337; 전량 ~9일 자율). **selector v2**: absent/big_move/rest_micro/hardcase 8·12·24·25→**전부 30**(evidence +162), lick/wheel **0**(coverage vs semantic 은 closure 후 확정; B1R 사람 retrieval 도 0→semantic 시사), overlap 0, pool `cec461e8`, manifest_emitted False. **다음(owner/새 handoff):** cap-500 라운드/자연 드레인으로 closure 완주→selector 최종재판정 / lick·wheel semantic 부재 확정 시 데이터수집 / live-lag 게이트 폴링주기 반영 재정의. 보고서 [`B1R2 report`](../docs/handoff-prompts/2026-07-22-local-vlm-evidence-b1r2-media-availability-report.md).
> **🟠 2026-07-22 Evidence B1R — verdict `B1R_BLOCKED_EVIDENCE_COVERAGE`(B2·모델 정지):** 두 원인 분리 검증. **① selector 굶김 = 해소 실증**: v2 multi-match+scarcity-first 가 v1 hardcase-first 흡수를 되돌려 current 2841 evidence 에서 absent final 0→24, rest 7→25 회복(hardcase 30→12). 독립 recompute MATCH, clip/episode overlap 0, pool SHA `614615b0`. **② coverage 완주 = 불가**: 30-clip canary 28/30 `r2_download_failed` — read-only R2 HEAD 표본으로 **보존창(~30일) 이전 clip 의 R2 object 삭제** 확인(06-17·06-22=0/6, 07-02~21=6/6). coverage OPEN(silent_missing 13915). `eligible=16786` 은 r2_key 존재만 봐 media 소실분 과대계상. lick/wheel raw=0 = semantic 부족도 잔존. canary 93% 실패로 대량 enqueue STOP(계약). nightly enqueuer missing-progress 하드닝 FF 배포(`a7635a9`, worker 무변경). **다음(owner 결정):** coverage audit 에 R2 존재검증 추가→분모 정정 / 보존창 이내 cohort 만 backfill / evidence-at-capture 검토. selector v2 는 회귀 기준점 보존. 보고서 [`B1R report`](../docs/handoff-prompts/2026-07-22-local-vlm-evidence-b1r-report.md).
