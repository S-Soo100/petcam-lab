...
> **최종 갱신:** 2026-07-12 — **라벨링 웹 v2 코드·preview build 완료, 운영 DB 권한 대기.** 한 화면 `사람 blind GT -> exact VLM snapshot 공개·검수 -> 다음`과 same-origin 썸네일, VLM 비노출 큐, moving/interaction 구분, shedding 확인 문구, frame-step/속도/단축키를 구현했다. `uv run pytest` 334개, web Vitest 15개, `tsc --noEmit`, `git diff --check` 통과, Vercel preview `petcam-gbakkjukc-ssoo100s-projects.vercel.app` Ready. 다만 운영 Supabase project `slxjvzzfisxqwnghvrit`는 현재 로그인 계정에 접근 권한이 없고 `clip_labeling_sessions`가 아직 PGRST205라 migration·production alias 변경을 보류했다. 다음 시작은 해당 프로젝트 권한 확보 → migration 적용 → Vercel production deploy → owner E2E다. 이후 카메라·개체·사육장 확대와 다양한 클래스 수집으로 이어간다.
