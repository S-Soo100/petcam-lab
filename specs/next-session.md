...
> **🟡 2026-07-28 R1 Mac mini runtime foundation 설계 승인 — `R1_DESIGN_APPROVED_MANIFEST_PENDING`:** Claude Desktop 독립 검토의 Critical 4건을 반영해 사람 주도 RUN-MANIFEST 구현 패키지와 무인 ledger-native job spec을 분리했다. R1은 `petcam-lab` 소유, Mac mini 전용 checkout `/Users/baek-end/petcam-lab-research-runtime`, local SQLite 현재 상태 + append-only JSONL 감사, `boot_id/pid/lease_epoch` fencing, max concurrency 1, production flock 비점유 probe+tracked quiet-window 양보, 쓰기 전 redaction, SSH pull-only 관측으로 고정한다. R1 실제 workload는 synthetic no-op뿐이며 dataset/media/model/DB/R2/VLM은 0이다. 다음은 설계·계획 A commit → start manifest 전용 M commit → `RUN_MANIFEST_OK --phase start`; 여기서 다시 정지하고 별도 구현 승인 뒤에만 B 구현을 시작한다. 정본 [`R1 설계`](../docs/superpowers/specs/2026-07-28-r1-mac-mini-runtime-foundation-design.md) · [`R1 구현계획`](../docs/superpowers/plans/2026-07-28-r1-mac-mini-runtime-foundation.md) · [`Claude 검토`](../docs/handoff-prompts/2026-07-28-r1-mac-mini-runtime-foundation-claude-review-report.md).
> **🟡 2026-07-27 RBA 연구 시스템 v1 방향 승인 — `RBA_RESEARCH_SYSTEM_V1_DESIGN_REVIEW`:** Mac mini를 연구 기준 실행 호스트로 고정하고 ChatGPT Desktop은 연구 설계·구현·해석, MacBook·스마트폰은 원격 지휘 역할로 분리한다. 연구 상태는 Git·중앙 카탈로그·dataset manifest·DB ledger에 보존한다. dataset-v1은 기존 203을 폐기하지 않고 `legacy_regression / development / challenge / fresh_holdout` 네 층으로 분리하며 episode+camera-night 누출을 금지한다. **실행 순서:** R1 Mac mini runtime foundation → R2 inventory → R3 split freeze → R4 current rebaseline → R5 candidate VLM → R6 evidence ablation → R7 local LLM 보조 → R8 prompt RC → R9 fresh holdout+shadow. 다음은 written spec 확인 후 R1 구현계획만 작성한다. 정본 [`RBA 연구 시스템 v1 설계`](../docs/superpowers/specs/2026-07-27-rba-research-system-v1-design.md).
> **🟢 2026-07-28 AI 연구 운영 계약 v1 검증 완료 — `AI_OPERATING_CONTRACT_V1_VERIFIED`:** A(base)→M(start manifest)→B(implementation)→C(final record) lifecycle, trusted P3/P4 approval, implementation host exact match, non-none runtime attestation, clean Git, RFC3339 schema/parser 경계를 fail-closed로 고정했다. 독립 final review는 Critical/Important/Minor 0 · Ready to merge Yes였고 focused 295 · 전체 1127 passed/3 skipped로 재검증했다. 다음 허용 단계는 R1 Mac mini runtime foundation의 설계·구현계획·start manifest 작성이며, 실제 runtime 구현과 service 설치는 승인된 R1 manifest/plan 및 별도 P3 gate 전에는 시작하지 않는다. 정본 [`AI 연구 운영 계약`](../docs/research/AI-OPERATING-CONTRACT.md) · [`검증 보고서`](../docs/handoff-prompts/2026-07-27-ai-operating-contract-report.md).
> **🟡 2026-07-27 Owner GT→Visibility ROI 연구 체인 마감 · consensus shadow 구현 대기 — `RESEARCH_CHAIN_CLOSED_CONSENSUS_SHADOW_PENDING`:** Owner GT 172건은 무결성·Python Evidence/Gate coverage 172/172로 유효하지만 2카메라·3일·39 episode·rare care 4라 production 일반화에는 부족하다. 실패한 것은 GT가 아니라 raw `roi_mean` 공통 신호 가설(`PE_MOTION_SIGNAL_INCONCLUSIVE`: pooled AUROC 0.7133, camera별 0.4832/0.8227)이다. legacy GT 통합 후 visibility 실패를 후보화했으나 현행 44-clip 재현에서 pass 간 변경 10, 동일 non-moving 상한 7로 Phase 1 기준 10에 도달 불가해 `VISIBILITY_ROI_REJECT_NO_CURRENT_REPRODUCIBLE_FAILURE`; read-only 교차검수 `CROSS_REVIEW_PASS`. **다음:** 사람 이중 블라인드 GT와 위험-label 3회 consensus shadow를 2~3주 병행하되 production 결과는 불변. 구현은 Lab ledger plan `afbffd67` Task 1부터 clean 재개→Nightly worker plan `bf701168`; migration/main/Mac mini 배포는 별도 승인. 정본 [`연구 정리`](../docs/handoff-prompts/2026-07-27-owner-gt-visibility-consensus-closure.md).
> **🟢 2026-07-25 짧은 오류 영상 visibility-first 운영 수용 — `SHORT_CLIP_VISIBILITY_FIRST_DEPLOYED_ACCEPTED`:** production migration·Fly/Vercel·app/web RLS·Mac mini `ENABLED=1/WRITE=1/DELETE=0` 적용과 RunAtLoad kickstart를 검증했다. 현재 원장 exclusions **823**(candidate 783 / quarantined 40), off-policy 격리 0, quarantined 40건은 기존 Owner `skip` 40건과 일치하며 `label/hold` 0, media_deleted 0, active delete lease 0이다. Owner 결정으로 추가 예약 감시와 자연 hourly cycle 1회 관측은 면제해 **미관측 사실을 보존한 채 운영 수용**으로 마감한다. R2 물리 삭제는 계속 비활성·범위 밖이다. 아래 `READY_FOR_DEPLOY` 블록은 구현·배포 전 이력으로 superseded.
