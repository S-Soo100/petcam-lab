...
> **후속 라벨링 문구 개선(2026-07-14):** 사람 판정 폼의 촬영환경 `해당 없음` 직접 확인, 휴식·급여 도구 등 실제 라벨러가 이해하기 어려웠던 문구를 쉬운 말로 정리했다. 저장 enum·DB·API 계약은 그대로이며 코드 커밋은 `67cef1b`다. 이 후속 커밋의 production 재배포·브라우저 검증 여부는 별도 확인 전까지 완료로 쓰지 않는다.
