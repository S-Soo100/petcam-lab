...
> **활동필터 v0 (2026-07-14, audit→fresh safety→문서보정 완료):** 파이프라인 + migration **3종** 적용(v0 / evidence_identity / identity_frames=**7컬럼**, FK=motion_clips, fail-open). 0.25 preflight=reject→**audit로 threshold 문제 규명**(conf 0.14~0.21 걸러진 것; Gate v3 는 일반화/segment 엔 여전히 필요). v1(0.10) 재실험 후 **fresh safety holdout(튜닝겹침 0·24, 카메라 A static 12+B absent 12)**: **exclude_absent = REJECT**(human=active 2건, 카메라 B, 0.10 도 게코 놓침=recall 잔존), **exclude_static = HOLD**(active 오제외 0 이나 **11/12가 07:22~07:41 UTC 한 에피소드=effective≈2** + 질문 결함). **⚠️ 사람 검수 질문이 presence(게코 유무)와 product outcome(제외/포함)을 혼합=질문 설계 결함**(static 12는 게코 실제 보임; owner 인지문제 아님). **3계층 내부 분리(presence/activity/product outcome), absent+static 은 outcome=exclude 에서만 합침.** utility(detector-only=upper bound): 표본합 36%≠운영, **clip 가중 27%**(static 17+absent 10). **두 차감 스위치 disabled(settings 0).** **다음(미실행, A~G)**: **A** policy-version guard(worker 가 settings.active_policy_version 과 config.ACTIVITY_POLICY_VERSION 비교, null/불일치→카메라 skip·미저장, 테스트 3케이스; config 기본 v0 유지·launchd 서 v1 명시)→**B** 커밋/push→**C** shadow-only(camera_activity_filter_settings enabled=true·두 exclude_*_enabled=false·active_policy_version='activity-v1', launchd env ACTIVITY_POLICY_VERSION=activity-v1, effective==raw 확인)→**D** 축적→**E** 30분 episode dedup 독립 static≥20→**F** 사람 blind(질문 2단분리 include/exclude 먼저)→**G** FE=0 시 카메라 A exclude_static 제한활성화. §15 Gate v3 backlog(B recall hardcase·A static calibration). utility 다음 스크립트=raw/excluded_duration_sec 저장. 커밋 gate `89237a5`·nightly `e5c9bdf`·lab `cd00472`(+보정 문서 uncommitted), **미push**. **금지: push/merge/launchd/Flutter/Gate v3/기존 migration 수정/DB write.** 상세 REPORT §8~§15 + 메모리 `activity-filter-v0-preflight-reject`.
