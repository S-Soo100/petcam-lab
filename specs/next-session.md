...
> **🟡 2026-07-31 Data Engine formal Blind30 인프라 배포 — `FORMAL30_INFRA_DEPLOYED_NO_RESERVATION` / `BLIND30_BLOCKED_REVIEWER_PAIR`:** [`TEST-SHEET`](../experiments/rba-data-engine-blind30/TEST-SHEET.md)의 fresh 30 selection, camera-night/5분 dedup, `motion-blind-v1`, segment IoU 0.50 또는 경계 2초, abstain/adjudication, pass/fail 계약과 exact 30/2 reviewers/60 slots/30 awaiting 원자 예약 RPC·독립 scorer를 production에 배포했다. 함수 실행 권한은 `service_role`만 허용하며 guard trigger와 unique index를 확인했다. formal30 cohort/slot/submission/consensus와 실제 selection/manifest/reservation/URL 생성은 모두 0이다. 배포 검증 중 일상 운영 live 제출 3건이 동시 유입돼 global slot/submission hash exact-equality는 관측할 수 없었고, consensus `18,474`건/hash `a019333756d28b273448abf29bf1944b` 불변과 formal30 subset 0으로 범위를 확인했다. 기존 paired 53건은 EDA, `owner-single-adopt-v1` 47건은 단일 reviewer provenance로만 보존한다. 다음은 같은 active group의 두 번째 qualified non-owner가 실제 tutorial-v1 5/5를 완료하는 사람 조치이며, 그 전에는 Task 7 표본 동결·예약을 실행하지 않는다. backlog 300 human-first Gate 감사는 별도 pending이다.
