...
> **최종 갱신: 2026-07-17 — S1R2 `S1R2_PASS_CROI_THROUGHPUT`(S2 allowed, 계획 작성만).** 비교용 A6 를 CROI 게이트에서 분리하고 질문을 **CROI/MPS/cold 단독 처리량**으로 좁힌 새 raw 시험. 고정 profile `croi-mps-cold`로 한 안전창에서 **96/96 measured 완주**(warmup32+measured96=128), CROI cold **p95 2.55s / cap 1,411.69/h = gate 160의 8.82배**, 독립 재계산 정확 일치, 운영 안전 전부 통과. **CROI production path 유효 확정.** feature push 완료. 이전 S1/S1R HOLD 는 이력 보존.
