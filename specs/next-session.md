...
> **최종 갱신:** 2026-07-12 — **라벨링 웹 v2 production 완료.** 운영 Supabase에 `clip_labeling_sessions` migration 적용, `label.tera-ai.uk` Vercel production Ready(`dpl_qU8VkyWPWUycoXd7vimgRwBJj6jF`). 운영 E2E에서 blind queue의 존재하지 않는 `camera_clips.ended_at` 조회를 발견해 commit `5479e4f`로 수정·재배포했다. 최종 확인: 30/30 썸네일 640px, 큐·GT 잠금 전 prediction 비노출, wheel interaction object/type 필수, initial GT 잠금 뒤 exact VLM snapshot 공개, incorrect+`action_confusion` 저장, 다음 clip 자동 이동. 임시 E2E user/profile/labeler/session/behavior_label은 전부 삭제해 운영 데이터 잔존 0. web Vitest 16개와 `tsc --noEmit` 통과. 다음 우선순위는 실제 owner 사용성 확인 → 카메라·개체·사육장 확대와 다양한 클래스 수집 → backlog Gate blind audit다.
