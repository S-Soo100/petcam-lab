...
> **활동필터 v0 (2026-07-14, audit→fresh safety 재검증 완료):** 파이프라인 + migration **3종** 적용(v0 / evidence_identity / identity_frames=**7컬럼** clip+model+schema+checkpoint_sha256+threshold+sampler_version+frames_sampled, FK=motion_clips, fail-open). 0.25 preflight=reject→**audit로 threshold 문제 규명**(0.25 가 conf 0.14~0.21 검출 걸러낸 것, Gate v3 불필요). v1(0.10+roi_flow 0.5) 재실험 후 **fresh safety holdout(튜닝겹침 0·24개, 카메라 A static 12+B absent 12)**: **exclude_absent = reject**(human=active 2건, 카메라 B, 0.10 도 게코 놓침=recall 잔존), **exclude_static = hold**(FE 0, 카메라 A 12, 단일카메라). **⚠️ owner 에게 absent≈static(둘 다 "제외") = 라벨 정의 이슈, SOT 서 통합 결정 필요.** utility(무작위·카메라×주야): raw 의 ~36% excludable, 카메라 편차 큼(A 24%/B 68%). 하드닝 6종+identity frames. **두 스위치 disabled(settings 0).** 커밋 gate `89237a5`·nightly `e5c9bdf`·lab `feat/activity-filter-migration`, **미push**. **금지: push/merge/launchd/Flutter/Gate v3/기존 migration 수정.** 상세 REPORT §8(audit)~§12(종합) + 메모리 `activity-filter-v0-preflight-reject`.
