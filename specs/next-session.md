...
> **🟡 2026-07-28 AI 연구 운영 계약 final-review 보정 — `AI_OPERATING_CONTRACT_IMPLEMENTED_AWAITING_FINAL_REVIEW`:** start manifest 자기참조 문제를 A(base)→M(start manifest)→B(implementation)→C(final record) lifecycle로 닫고, P3/P4 trusted approval verifier·implementation host exact match·non-none runtime attestation·clean Git·schema/parser parity를 fail-closed로 보강했다. 현재 판정은 `IMPLEMENTED_AWAITING_FINAL_REVIEW`이며 이전 verified 주장은 철회한다. controller의 독립 final review와 후속 report-only 승격 commit 전에는 R1 plan·runtime 구현·장기 job을 시작하지 않는다. 정본 [`AI 연구 운영 계약`](../docs/research/AI-OPERATING-CONTRACT.md) · [`보정 보고서`](../docs/handoff-prompts/2026-07-27-ai-operating-contract-report.md).
