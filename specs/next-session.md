...
- **P3 = T1 highlight-selection probe (실행 중):** DB-only 합성점수(존재+활동+주기성 백분위 평균, 버킷 캡) top20 vs random20, owner blind 판정, LLM 0회. TEST-SHEET 승인·동결 2026-07-21: [`experiments/t1-highlight-selection/TEST-SHEET.md`](../experiments/t1-highlight-selection/TEST-SHEET.md). 게이트: Δ≥+20%p AND S informative≥8/20 → adopt(selector 입력 스펙 승격 제안). 부산물 blind 판정 40건 = 사람 GT 적립.
