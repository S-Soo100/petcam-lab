...
> **🟡 2026-07-31 Data Engine formal Blind30 — `BLIND30_RESERVATION_APPROVED_PREFLIGHT_READY`:** [`TEST-SHEET`](../experiments/rba-data-engine-blind30/TEST-SHEET.md)의 threshold·selection·`motion-blind-v1` 계약과 exact 30/2 reviewers/60 slots/30 awaiting 원자 예약 RPC·독립 scorer를 production에 배포했다. B그룹의 서로 다른 두 non-owner가 approved/current active/current-run tutorial 5/5/waiver 0이고 membership history 각 1행·later reassignment 0이라 reviewer gate를 충족한다. 그룹 배정과 tutorial 완료의 시간 선후는 계약 조건이 아니다. dry-run은 eligible `15,671`, exact 30, camera 3, camera-night 30, selected 5-minute duplicate 0이며 formal30 cohort/slot/submission/consensus는 `0/0/0/0`이다. Claude 독립 효과성 심사 `EFFECTIVE/GO`와 owner Task 7 실행 승인을 확보했다. PASS의 인증 범위는 natural production distribution의 2인 일치도·abstain·owner adjudication 부담·운영무결성뿐이며 희소행동, taxonomy, train/validation GT, 모델/VLM/Gate/router/P0, chance-corrected agreement는 인증하지 않는다. 실제 class 분포는 descriptive-only이며 rare challenge는 별도 미래 TEST-SHEET다. 다음은 새 실 T0로 exact 30을 동결해 RPC를 정확히 한 번 호출하고 human 제출에서 멈추는 것이다. backlog 300 human-first Gate 감사는 별도 pending이다.
