...
> **활동필터 v0 (2026-07-14, audit로 뒤집힘):** 파이프라인 완성 + migration 2종 운영적용(`clip_prelabels`/`clip_activity_assessments`/`camera_activity_filter_settings`+`v_clip_effective_activity`, FK=motion_clips, fail-open). **초기 0.25 preflight=두 스위치 reject였으나 threshold audit로 철회** — 0.25 가 conf 0.14~0.21 gecko 검출을 걸러낸 것(detector FN 아님, **Gate v3 재학습 불필요**). 하드닝 6종(indexer starvation→pagination / evidence identity 6컬럼 / policy reuse / self-healing done=assessment / no-gecko+global→unknown / versioned preset) + **activity-v1**(thr 0.10+roi_flow 0.5)서 static 30·absent 4(카메라 A absent 1.25% 희소) **FE 0·precision 100% 조건부 adopt**. 커밋 gate `89237a5`·nightly `6c26630`·lab `a57cabb`, **미push**. **금지(사용자): push/merge/launchd/Flutter/Gate v3 재학습/기존 migration 수정.** 실제 Phase 5 활성화=사용자 승인+표본 축적 후. 상세 REPORT §8(audit)·§9(v1 판정) + 메모리 `activity-filter-v0-preflight-reject`.
