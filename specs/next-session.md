...
> **🟠 2026-07-31 Data Engine formal blind 30 — 계약 동결, 실행 blocker 2건:** [`TEST-SHEET`](../experiments/rba-data-engine-blind30/TEST-SHEET.md)에 fresh 30 selection, camera-night/5분 dedup, `motion-blind-v1`, segment IoU 0.50 또는 경계 2초, abstain/adjudication, pass/fail을 고정했다. 기존 paired 53건은 EDA, `owner-single-adopt-v1` 47건은 단일 reviewer provenance로만 보존하며 재사용하지 않는다. 현재 tutorial 완료 non-owner 두 명이 서로 다른 active group이고 generic canary는 20개 상한이라 exact 30 원자 예약이 불가능하다. manifest/cohort/slot/submission 생성은 0이며 판정은 `BLIND30_PREFROZEN_BLOCKED_REVIEWER_PAIR_AND_EXACT30_RESERVATION`이다. 다음은 같은 group의 두 번째 실제 tutorial 5/5와 별도 exact-30 RPC TDD다. backlog 300 human-first Gate 감사는 별도 pending이다.
