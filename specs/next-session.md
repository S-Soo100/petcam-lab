...
> **🟡 2026-07-30 Data Engine 다음 착수점 — 공통 튜토리얼 5개 + 실제 라벨러 1명 pilot:** tutorial-v1의 production 배포·5개 seed·격리는 이미 완료됐다. 다음은 지정 라벨러 1명이 5/5를 수행하고 첫 본작업 5개까지 확인하면서 총/lesson 라벨링 시간, uncertain 사용, tutorial correction과 운영 재검수 비율을 측정하는 단계다. 설계·실행계획은 [`tutorial pilot 설계`](../docs/superpowers/specs/2026-07-30-rba-data-engine-tutorial-pilot-design.md)와 [`tutorial pilot 계획`](../docs/superpowers/plans/2026-07-30-rba-data-engine-tutorial-pilot.md)이며, 그 결과를 보고 난 뒤 공통 blind 30 계약을 별도 동결한다.
> **🟢 2026-07-30 owner tutorial smoke — `OWNER_TUTORIAL_SMOKE_COMPLETED`, formal pilot pending:** fingerprint `05fd4fe03dc3` owner 계정이 production `tutorial-v1` run 1의 position 1~5를 모두 완료했다(22:53:10~22:56:36 KST, waiver 0). 화면·교육·저장·완료 전환 smoke로는 유효하지만 owner는 `labelers`와 tutorial gate를 우회하므로 실제 non-owner 라벨러 pilot이나 위 체크박스 완료로 세지 않는다. owner first-5 workflow smoke는 2026-07-30 KST `unreviewed`+media-ready 큐 최소 5개로 준비됐으며, 자기 결과의 owner 재확인은 독립 재검수 지표에서 제외한다. formal non-owner pilot과 독립 correction/approval 측정은 계속 대기다.
