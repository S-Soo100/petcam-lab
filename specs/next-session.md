...
> **최종 갱신:** 2026-07-12 — **라벨링 상세 촬영정보·원본 다운로드 구현.** 상세 상단에 `camera_clips.started_at` KST 촬영 일시와 반올림 duration을 표시하고, 기존 clip 권한을 통과한 attachment R2 signed URL로 원본 MP4를 직접 다운로드한다. 파일명은 `petcam_YYYY-MM-DD_HHmmss_<clip-id 8자리>.mp4`. 기존 Labeling Web v2 production 계약과 blind GT 흐름은 유지한다. Web Vitest 18개·TypeScript 통과; production 배포/E2E 증거는 아래 구현 계획에 기록한다. 상세 [설계](../docs/superpowers/specs/2026-07-12-labeling-video-header-download-design.md) · [구현 계획](../docs/superpowers/plans/2026-07-12-labeling-video-header-download.md).
