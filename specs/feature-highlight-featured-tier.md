...
**v0.1 (owner 2026-09-11, "3개는 적다 · 시간당 3개쯤으로 조금 줄이고 지켜보자"):** 사건 묶기 30분→**10분**, 하루 상한 3→**없음**, 대신 **같은 시간대(대표 클립의 KST 시) 안 최대 3개**. 실측(최근 14일 주 카메라) 하룻밤 11개·최대 16. 후보 조합 실측: 묶기 없음+시간당 3 → 17/22, 10분+시간당 2 → 10/14, 10분+하루 10 → 10/10. migration `2026-09-11_highlight_featured_hour_cap.sql`(`p_hour_cap` 추가, DROP+CREATE). 일주일 뒤 `scripts/report_highlight_featured.py` 로 재확인.
