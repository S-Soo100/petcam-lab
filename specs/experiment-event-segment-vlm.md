...
### 1.2 Selective fallback 원칙

초기 검증 단계에서는 SegmentVLM 을 전체 영상에 돌리지 않는다. VT label 이 있는 동안은 **Track A 결과와 VT label 이 불일치한 항목만 Track B로 넘기는 selective fallback** 으로 운영한다.

```text
VT label
  vs
Track A Zero-shot VLM prediction
  ↓
일치 → Track A 결과 유지
불일치 → SegmentVLM 재분석 artifact 생성
```

이 방식의 목적은 Track B의 절대 성능을 바로 주장하는 것이 아니라, **Track A 오답 중 SegmentVLM 이 얼마나 회복하는지**를 측정하는 것이다.

측정 항목:
- Track A mismatch 수
- SegmentVLM 실행 clip 수
- SegmentVLM recovered / recovered_but_review / still_wrong 비율
- 라벨별 회복률 (`defecating`, `drinking`, `shedding`, `eating_prey` 우선)
- 평균 event 수와 평균 event 길이
- Track A 대비 비용 배수
- human review 로 남겨야 하는 비율

비용 원칙:
- Track A는 전체 clip에 대해 1회 실행하는 production baseline 으로 둔다.
- Track B는 mismatch clip에만 실행한다.
- SegmentVLM 은 event 수만큼 모델 호출이 늘 수 있으므로, 전체 적용이 아니라 fallback 적용을 기본값으로 둔다.
- 운영 단계에서 VT label 이 없어지면, 이 실험 결과를 바탕으로 `low confidence`, `confusion-prone label`, `user flagged clip`, `high-value P0 candidate` 같은 fallback trigger 를 별도로 정의한다.
...
- VT label 단계에서는 `VT label != Track A prediction` mismatch 항목만 SegmentVLM selective fallback 으로 처리
- 산출물은 `experiments/segment-vlm/` 아래 JSONL/리포트로 저장
...
- [ ] `experiments/segment-vlm/roi-config.example.json` 작성 — normalized ROI config 예시
