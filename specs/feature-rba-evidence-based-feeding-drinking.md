...
> **⚠️ "YOLO bbox crop → VLM 재판정" 경로 기각** (`experiments/roi-crop-center/`, 2026-06-16): "입/먹이 영역을 crop 확대해 VLM에 다시 넣으면 정확도↑" 가설을 직접 측정 → center crop 4K급 10건 **순 0**(고해상 풀디테일을 줘도 회복 0). 즉 YOLO로 ROI를 잡아 VLM 재판정으로 보강하는 길은 헤드룸이 없다. **YOLO의 역할은 좌표·ROI 체류 evidence 생성 + 투입 컨텍스트 매칭으로만** — VLM 정확도 보강 용도가 아니다.
