...
|---|---|---|---|---|
| frames-10 (구 기준) | 10 | 1024×576 | ~7.9k (786×10) | full-202: Sonnet 78.2% / Opus 81.2% (Fable 85.1% 참고) |
| **frames 적응형@1080 (v4.0 신표준)** | 6~20 (간격3.5·구간중앙) | min(원본,1080) | ~12k (평균11.1장) | **185 Sonnet v4.0 85.9% adopt** (2026-06-13, `v40-regression/`) — 고정10 뒷부분손실 버그 대체 |
...
### M0 결과 (Sonnet 12변형 × 20건, 2026-06-12) — **decision: hold (신호 부정적)**

배치: Sonnet 4.6 서브에이전트 12개 병렬, ~770k tok / ~3.5분. 채점 `_score_repr.py --phase M0`
(검증 PASS — schema/missing/duplicate/mismatch 0). **frames 동일 20건 기준선 = 12/20.**

| variant | raw/20 | micro12 | prio3/11 | rec | brk | imgTok |
|---|---|---|---|---|---|---|
...
**판독:**
- **어떤 변형도 frames(12/20)를 못 넘음.** 최고 18f-2s-nots도 11/20 (−1) 이고 broken 4 > recovered 3.
  → 게이트(−3%p 이내) 관점에선 18f-2s-nots만 경계선, 나머지는 명확 미달.
- **셀 해상도 = 레버 확인**: 2장분할 평균 **8.7/20** vs 단일장 평균 **5.7/20** (+3.0). 단일장 중
  셀이 가장 작은 20f-1s가 최악(3~5). 0608 "셀 72px 패인" 가설을 같은 방향으로 지지하나,
  셀을 683px(2장)로 키워도 frames(1024px) 천장을 못 넘음.
- **micro12(미세접촉) 붕괴**: 최고 4/12 (frames 동일셋 6/12). 혀-표면 접촉은 몽타주 셀에서
  소실 — P2 "게이팅=공간해상도" 재확인. 몽타주는 미세접촉에 구조적 부적합.
- **ts on/off 무신호**: 18f-2s nots>ts, 16f-2s ts>nots, 12f-2s 동률 — 평균 차 ≈0. 타임스탬프는
  레버 아님 (작은 셀에선 텍스트가 오히려 영상 일부 가림).
- discordant 128건(`discordant_queue.json`) — 몽타주는 frames와 "조금 다른" 게 아니라 판정이
  크게 갈림(변형당 평균 ~10.7건 불일치).

**decision = hold.** M0는 스크리닝(20건, ±1=5%p 노이즈)이라 단독 기각 금지(§4-3a). 하지만 신호는
부정적 — 셀 해상도를 최대로 키운 2장분할도 frames 미달 + micro 붕괴. **M1 진출 = 상위 1개
`mv2-18f-2s-nots` 만** micro55+Opus로 확정 측정해 매듭 (반등 기대 낮음). 입력표현 천장은
frames가 잡고 있다는 누적 결론(P2)과 정합. → **cv-frames(V1)는 별개 트랙으로 유효**(몽타주 아닌
개별 프레임 밀도라 미세접촉 소실 문제에서 자유 — drinking 한정 확인).
