...
| 🚧 | [feature-rba-data-engine-v1.md](feature-rba-data-engine-v1.md) | **2026-08-03 GME_OWNER_APPROVED.** 사람 GT 기반은 유지하고 local VLM/router·자동 사건 묶기·Claude CLI 영상 판독은 종료한다. Python 시각 파이프라인은 [`Gecko Motion Engine`](../docs/superpowers/specs/2026-08-03-gecko-motion-engine-v1-design.md)으로 재정의해 Gate 기반 검출·추적과 실제 게코 움직인 시간을 맡는다. 현재 순서는 R2 초기 무효 영상 정리 → Dataset v2 → GME offline baseline → OpenAI API 행동·관찰 pilot이며 자동 skip·자동 병합은 금지한다 |
