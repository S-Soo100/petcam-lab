...
**[→ 연구 중앙 카탈로그](../docs/research/README.md)** — 여러 레포에 흩어진 실험의 판정·원문·재개 조건과 보존 정책을 찾을 때 사용.
...
| 🚧 | [feature-rba-data-engine-v1.md](feature-rba-data-engine-v1.md) | **라벨링 웹 v2 production 완료 / 데이터 확대 대기.** blind GT→exact VLM 검수, 썸네일 30/30, interaction validation, 다음 이동 E2E 완료. 334+16 tests/tsc 통과. 이후 실제 owner 사용성 확인 → 카메라·개체·사육장 확대 → Gate v3 shadow 순서 |
| 📝 | [2026-07-24 짧은 영상 장치 오류 격리 설계](../docs/superpowers/specs/2026-07-24-short-clip-device-error-retention-design.md) | 전 카메라 `<15초`는 후보 신호, 검증된 카메라별 길이만 자동 격리. 7일 복구 기간과 사람 GT 보호 가드 뒤 R2 객체만 삭제 |
