...
| 🚧 | [feature-api-server-fly-deploy.md](feature-api-server-fly-deploy.md) | **2026-05-08 Phase 1 + Phase 2 ✅, Phase 3 (DNS cutover) 대기** — API 서버 (`api.tera-ai.uk` → fly.io `petcam-api.fly.dev`) 이전 + Flutter contract 누락 endpoint 2개 (`/me/is_labeler`, `/clips/highlights`). Phase 1 commit `b458bb0` (pytest 247). Phase 2 staging 가동 (nrt, shared-cpu-1x 256MB, /health 200, 1대 scale-down). 회고: opencv-python → headless (X11 deps), secrets import stdin pipe 이슈 우회 |
