...
| 🚧 | [feature-rba-data-engine-v1.md](feature-rba-data-engine-v1.md) | **tutorial onboarding verified / double-blind operational / v1 `INVALID_SAMPLE_AFTER_FREEZE`.** v1 frozen 30 중 실제 R2 object 5개 부재를 확인해 submission 0 상태로 비파괴 close했다. 기존 60 slots/30 awaiting/manifest는 보존한다. v2는 future pool과 실제 R2 HEAD 이중 preflight, 새 `b30v2` RPC로 재동결 준비 중이다. threshold/comparator는 불변이며 기존 paired 53은 EDA, single-adopt 47은 agreement에서 제외. 이후 제품 방향은 [`사건 단위 전수 분석`](../docs/superpowers/specs/2026-07-31-rba-event-first-total-coverage-design.md)으로 확정했고 Phase 1 shadow 구현 계획까지 동결했다 |
...
| 🟡 | [router-cost-v2/TEST-SHEET.md](../experiments/router-cost-v2/TEST-SHEET.md) | **실행 전 / 데이터 엔진 이후.** 저비용 API baseline과 router policy를 동결하고 이후 future camera-night만 평가. 14박·300 clips, P0 30 pilot / 150 adoption, total eventual KRW·P0 recall·review burden·delay 동시 gate |
