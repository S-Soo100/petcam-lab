...
**상태:** ✅ **Phase 0 완료** (2026-06-20 맥미니 재부팅 생존 검증 통과 → 검증 종료로 스모크 bootout. launchd PATH 버그 수정 `c3f04e2`)
...
**맥북 검증 ✅ 6/6 + 맥미니 검증 ✅ (2026-06-20).** 재부팅 생존까지 통과 → Phase 0 종료.
...
> **✅ 맥미니 검증 완료 (2026-06-20)**: clone(PRIVATE→gh auth) → `uv sync` → `.env`(3키) → smoke ✅ → `install-launchd.sh` → **launchd PATH 버그**(uv=`~/.local/bin`·claude=`/opt/homebrew/bin` 분리 → plist PATH에 claude 누락 → `No such file 'claude'`) 발견·수정·push(`c3f04e2`) → **재부팅+자동로그인 생존 ✅**(23:01 KST) → 구독 한도 초과 확인 후 검증 종료, 스모크 bootout. Phase 0 종료.
...
### ⚠️ Phase 1 진입 전 해소 — claude 구독 한도 분리 (2026-06-20 발견)

Phase 0 검증 중 발견: 맥미니 5분 폴링(하루 288회 `claude -p`)이 **사용자 본인 claude 작업과 같은 구독 한도를 공유** → 소진 충돌(재부팅 후 첫 1회만 ✅, 이후 `You've hit your session limit`, 1:10am 리셋 후 복귀로 확증). 코드·PATH·keychain 무죄, 순수 사용량 문제. gate·nightly-reporter 등 **모든 맥미니 워커 공통 제약**(같은 claude 구독 쓰면 합산 소진).

Phase 1 워커 가동 전 한도 분리 설계 선행:
- **A. 전용 claude 계정/구독** — 워커 전용 분리. 비용 ↑
- **B. 종량제 API key** (`ANTHROPIC_API_KEY`) — 구독과 예산 독립. 호출당 과금
- **C. 저빈도 설계** — 5분은 스모크용. 실워커(nightly=야간 1회)는 애초에 저빈도라 충돌 적음

(메모리 `claude-subscription-quota-shared`)
